import "./style.css";
import { jsxs as W, jsx as l, Fragment as oA } from "react/jsx-runtime";
import * as J from "react";
import { forwardRef as cA, createContext as BA, useContext as vA, Fragment as UA, useRef as eA, useState as z, useCallback as T, useEffect as Z, isValidElement as ae, createElement as Ae, useMemo as fA } from "react";
import He from "react-dom/server";
const V = {
  AUDIO_BUTTON: "audio-button",
  NOTIFICATION_BUTTON: "notification-button",
  CLOSE_CHAT_BUTTON: "close-chat-button",
  SEND_MESSAGE_BUTTON: "send-button",
  VOICE_MESSAGE_BUTTON: "voice-button",
  FILE_ATTACHMENT_BUTTON: "file-attachment-button",
  EMOJI_PICKER_BUTTON: "emoji-button"
}, Oe = "data:image/svg+xml,%3csvg%20xmlns='http://www.w3.org/2000/svg'%20width='32'%20height='32'%20viewBox='0%200%2042%2042'%3e%3cpath%20style='fill:none;stroke-width:2;stroke-linecap:butt;stroke-linejoin:miter;stroke:%23b00000;stroke-opacity:1;stroke-miterlimit:4'%20d='M17%209a8%208%200%200%201-8%208%208%208%200%200%201-8-8%208%208%200%200%201%208-8%208%208%200%200%201%208%208Zm0%200'%20transform='rotate(.086)%20scale(1.77778)'/%3e%3cpath%20style='fill:none;stroke-width:2;stroke-linecap:butt;stroke-linejoin:miter;stroke:%23b00000;stroke-opacity:1;stroke-miterlimit:10'%20d='M14.66%2014.66%203.34%203.34'%20transform='rotate(.086)%20scale(1.77778)'/%3e%3c/svg%3e", ee = "data:image/svg+xml,%3csvg%20xmlns='http://www.w3.org/2000/svg'%20xml:space='preserve'%20viewBox='0%200%201000%201000'%3e%3crect%20width='100%25'%20height='100%25'%20fill='%23fff'/%3e%3cg%3e%3crect%20width='45'%20height='30'%20x='-22.5'%20y='-15'%20rx='1.5'%20ry='1.5'%20style='stroke:none;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%2337547a;fill-rule:nonzero;opacity:1'%20transform='rotate(90%20-99%20362)%20scale(3.4003)'/%3e%3cpath%20d='M0%2075q14%200%2014%2017%200%2016-14%2016Z'%20style='stroke:none;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%2398b0ca;fill-rule:nonzero;opacity:1'%20transform='rotate(180%20106%20386)%20scale(3.4003)'/%3e%3crect%20width='25'%20height='4'%20x='-12.5'%20y='-2'%20rx='.2'%20ry='.2'%20style='stroke:%2346648c;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%2346648c;fill-rule:nonzero;opacity:1'%20transform='rotate(90%20-46%20282)%20scale(3.4003)'/%3e%3cpath%20d='M19%2065h4l5%204v1H14v-1Z'%20style='stroke:none;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%2346648c;fill-rule:nonzero;opacity:1'%20transform='translate(165%20150)%20scale(3.4003)'/%3e%3crect%20width='25'%20height='2'%20x='-12.5'%20y='-1'%20rx='.1'%20ry='.1'%20style='stroke:%2398b0ca;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%2398b0ca;fill-rule:nonzero;opacity:1'%20transform='rotate(90%20-3%20240)%20scale(3.4003)'/%3e%3ccircle%20r='7.5'%20style='stroke:none;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%23a478fc;fill-rule:nonzero;opacity:1'%20transform='translate(236%20175)%20scale(3.4003)'/%3e%3crect%20width='45'%20height='30'%20x='-22.5'%20y='-15'%20rx='1.5'%20ry='1.5'%20style='stroke:none;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%2337547a;fill-rule:nonzero;opacity:1'%20transform='rotate(90%20139%20600)%20scale(3.4003)'/%3e%3cpath%20d='M184%2075q14%200%2014%2017%200%2016-14%2016Z'%20style='stroke:none;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%2398b0ca;fill-rule:nonzero;opacity:1'%20transform='translate(165%20150)%20scale(3.4003)'/%3e%3crect%20width='25'%20height='4'%20x='-12.5'%20y='-2'%20rx='.2'%20ry='.2'%20style='stroke:%2346648c;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%2346648c;fill-rule:nonzero;opacity:1'%20transform='rotate(90%20219%20548)%20scale(3.4003)'/%3e%3cpath%20d='M175%2065h4l5%204v1h-14v-1Z'%20style='stroke:none;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%2346648c;fill-rule:nonzero;opacity:1'%20transform='translate(165%20150)%20scale(3.4003)'/%3e%3crect%20width='25'%20height='2'%20x='-12.5'%20y='-1'%20rx='.1'%20ry='.1'%20style='stroke:%2398b0ca;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%2398b0ca;fill-rule:nonzero;opacity:1'%20transform='rotate(90%20262%20505)%20scale(3.4003)'/%3e%3ccircle%20r='7.5'%20style='stroke:none;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%23a478fc;fill-rule:nonzero;opacity:1'%20transform='translate(767%20175)%20scale(3.4003)'/%3e%3crect%20width='116.7'%20height='70'%20x='-58.4'%20y='-35'%20rx='31.5'%20ry='31.5'%20style='stroke:none;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%2371c9fc;fill-rule:nonzero;opacity:1'%20transform='translate(502%20877)%20scale(3.4003)'/%3e%3crect%20width='60'%20height='20'%20x='-30'%20y='-10'%20rx='3'%20ry='3'%20style='stroke:none;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%2337547a;fill-rule:nonzero;opacity:1'%20transform='translate(502%20728)%20scale(3.4003)'/%3e%3ccircle%20r='75'%20style='stroke:none;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%2371c9fc;fill-rule:nonzero;opacity:1'%20transform='translate(502%20473)%20scale(3.4003)'/%3e%3crect%20width='129.4'%20height='82.3'%20x='-64.7'%20y='-41.1'%20rx='37'%20ry='37'%20style='stroke:none;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%23fff;fill-rule:nonzero;opacity:1'%20transform='translate(502%20464)%20scale(3.4003)'/%3e%3crect%20width='108'%20height='68.6'%20x='-54'%20y='-34.3'%20rx='30.9'%20ry='30.9'%20style='stroke:none;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%23162334;fill-rule:nonzero;opacity:1'%20transform='translate(502%20464)%20scale(3.4003)'/%3e%3ccircle%20r='19.4'%20style='stroke:none;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%23a478fc;fill-rule:nonzero;opacity:1'%20transform='translate(415%20464)%20scale(3.4003)'/%3e%3ccircle%20r='19.4'%20style='stroke:none;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%23a478fc;fill-rule:nonzero;opacity:1'%20transform='translate(591%20464)%20scale(3.4003)'/%3e%3crect%20width='96'%20height='53.2'%20x='-48'%20y='-26.6'%20rx='23.9'%20ry='23.9'%20style='stroke:%23162334;stroke-width:1;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:4;is-custom-font:none;font-file-url:none;fill:%23162334;fill-rule:nonzero;opacity:1'%20transform='translate(503%20875)%20scale(3.4003)'/%3e%3cpath%20stroke-linecap='round'%20d='m68%20215%207-15M81%20222l-7-22M81%20222l7-14M101%20208H87M114%20219l-7-24M113%20219l13-22M131%20214l-5-17M148%20214h-18M69%20214H51M100%20209l7-15'%20style='stroke:%23a478fc;stroke-width:3;stroke-dasharray:none;stroke-linecap:butt;stroke-dashoffset:0;stroke-linejoin:miter;stroke-miterlimit:10;is-custom-font:none;font-file-url:none;fill:none;fill-rule:nonzero;opacity:1'%20transform='translate(165%20150)%20scale(3.4003)'/%3e%3c/g%3e%3c/svg%3e", xe = "data:image/svg+xml,%3csvg%20width='140'%20height='140'%20xmlns='http://www.w3.org/2000/svg'%3e%3ccircle%20cy='60.044'%20cx='70'%20fill='%235c5c5c'%20r='29'/%3e%3cpath%20d='m16.36%20156.186-.01-30.5c-.005-16.844%2024.168-30.507%2053.991-30.516%2029.823-.009%2054.004%2013.64%2054.01%2030.484l.008%2030.5-108%20.032z'%20fill='%235c5c5c'/%3e%3c/svg%3e", Le = (A, e) => /* @__PURE__ */ J.createElement("svg", { viewBox: "0 0 637 637", ref: e, ...A }, /* @__PURE__ */ J.createElement("path", { d: "m478 113-13 2-4 1h4l14-2c10-2 35-2 42-1l6 3 3 1c3 0-4-3-8-5-7-1-34-1-44 1m56 6 8 6c7 5 20 17 20 19s7 12 8 12l-3-6-4-6c0-3-15-17-25-24-5-3-8-4-4-1m-198 16-9 1a512 512 0 0 1 9-1m-180 30a412 412 0 0 0-23 4 167 167 0 0 0-32 8 439 439 0 0 1 46-9 304 304 0 0 1 11-2l6-1h-8m369 8 4 3-7-6c-1 0 0 2 3 3m-439 9c-7 3-18 13-18 15l6-5c5-5 9-8 14-10l6-4-8 4m442 12-10 11-7 6 7-6c7-5 14-14 13-16l-3 5M65 205l-1 11c0 9 0 10 4 17 5 10 7 12 22 11 10 0 15 0 38-5l5-1-6 1-16 2-21 2c-13 1-16-1-21-10-3-5-4-6-4-15l1-12c1-4 1-5-1-1m335 1c3 0 3 1 3 3l-2 6c-2 5-35 39-76 80-53 51-120 117-138 137-21 24-22 25-22 27 0 3 6 9 9 9 4 0 14-8 38-27a5073 5073 0 0 1 15-16 1421 1421 0 0 0-17 15l-31 25-6 2-4-3c-3-3-3-4-2-6 3-8 79-85 160-163 57-57 79-80 79-86 0-2-3-4-7-4-2 0-2 0 1 1m165 18c-9 17-25 32-57 53l-10 8 16-11 12-8c15-10 27-22 39-39 5-7 6-8 4-8l-4 5m-221 2a199 199 0 0 0-19 10 183 183 0 0 1 27-12c0-1-1-1-8 2m-34 16c-3 3-2 3 2 0l1-2-3 2m151 3-1 2 2-2c3-3 2-3-1 0m-16 12-11 10a281 281 0 0 0 11-10m-24 18-7 5-7 5-42 31c-10 7-24 19-22 19l7-6 16-12a343 343 0 0 0 35-25 573 573 0 0 0 20-17m-187 0-2 2 4-2 3-2-5 2m-14 6-3 2 4-1 4-3-5 2m-7 3-8 4-18 8 18-7 7-3 3-1c1-2 0-2-2-1m254 24a2672 2672 0 0 1-71 54 601 601 0 0 0 39-29 1352 1352 0 0 1 32-25m-291-8-2 2 4-2 3-2-5 2m-13 5-2 2h3c5-3 4-4-1-2m-19 9-7 3-13 6c-10 5-13 7-14 10-3 4-2 5 1 0 2-3 5-5 14-9l13-6 12-6-6 2m178 36c-17 12-21 16-21 17a390 390 0 0 1 20-17m-206 34 1 4 1 3 1 5 2 8 1 5v-6l-1-8-2-5-1-4-1-2c-1-2-1-2-1 0m222 23-9 7a683 683 0 0 0-56 44 261 261 0 0 0-26 20 1345 1345 0 0 0-46 34c-6 3-7 3-19 3-11 0-12 0-18-3-8-4-10-8-17-20l-5-10 4 10c5 10 12 18 19 22 4 2 6 2 17 2 13 0 14 0 21-3l26-19 42-33a2183 2183 0 0 0 43-34 643 643 0 0 1 24-20", fill: "#fff", fillRule: "evenodd" }), /* @__PURE__ */ J.createElement("path", { d: "M478 114a620 620 0 0 1-39 6l-21 3a5481 5481 0 0 0-19 3 870 870 0 0 1-54 8l-1 1-6 1-12 2-13 2a992 992 0 0 0-76 12 400 400 0 0 1-45 8l-6 1-5 1h-3l-3 1-2 1-26 4-13 2-17 3c-23 5-33 9-43 19-7 8-9 13-9 27 0 8 1 9 4 14 5 9 8 11 21 10l21-2a1133 1133 0 0 1 23-4 659 659 0 0 0 74-14l3-1h3l3-1h3l2-1 2-1h3l2-1h2l24-5 38-7a2100 2100 0 0 1 93-18l3-1 10-1a2082 2082 0 0 1 120-17l2 1c2 0 8 5 8 7l1 2 1 6c0 4 0 6-3 10-2 4-16 16-18 16l-7 5-10 8a1070 1070 0 0 0-11 7 113 113 0 0 1-20 14l-2 2-3 2-2 2h-1a154 154 0 0 1-28 21l-5 4-5 4a573 573 0 0 1-50 37 175 175 0 0 0-24 18l-1 2-2 1-2 1-2 1-1 1-12 10a396 396 0 0 0-32 25l-8 6-6 5-9 7a326 326 0 0 1-25 20l-13 12a5073 5073 0 0 0-17 15c-24 20-34 27-39 27-2 0-8-6-8-9 0-2 1-3 22-27 18-20 85-86 138-137 41-41 74-75 76-80l2-6c0-2 0-3-2-3l-7 1a233 233 0 0 1-26 9l-1 1-1 1h-1l-1 1h-2l-3 2-1 1h-2l-3 2h-1l-5 2-22 10-9 4-4 2-5 2-10 4-2 1-2 1-2 1-13 6a334 334 0 0 1-38 17 75 75 0 0 0-14 6l-5 2-4 1-2 1-3 2-7 3-16 6a247 247 0 0 0-32 14l-1 1h-2l-1 1h-2l-1 1-12 6a103 103 0 0 0-27 15l-2 5-1 1v11l1 5 1 5 1 4c0 3 0 4 2 4v3l1 3 1 2 2 10 1 4 1 5 2 10 2 8 1 4 1 4c3 3 9 25 11 38l1 7v2l1 4v1l1 1v2l1 2 1 3v1l5 10c7 13 9 16 17 20 6 3 7 3 18 3 12 0 13 0 19-3 8-4 11-6 26-18a1345 1345 0 0 1 85-67 683 683 0 0 1 50-39l12-9a513 513 0 0 1 44-33 201 201 0 0 0 22-17 2270 2270 0 0 0 73-54 158 158 0 0 0 58-59c2 0 3-9 4-27l-1-19-1-4-1-6-1-3-4-6-4-6c0-2-6-8-13-14a77 77 0 0 0-28-18c-6-1-34-1-43 1" })), te = cA(Le), Ne = (A, e) => /* @__PURE__ */ J.createElement("svg", { viewBox: "0 -960 960 960", ref: e, ...A }, /* @__PURE__ */ J.createElement("path", { d: "M765-321q0 117-84 196-83 80-200 80-118 0-200-80t-83-196v-392q0-85 61-144 61-58 146-58 86 0 147 58 61 59 61 144v374q0 54-39 92-39 37-93 37-55 0-93-37-38-38-38-92v-372h110v372q0 9 6 14t15 5q8 0 15-5t7-14v-374q0-39-29-66t-68-27q-40 0-69 27t-29 66v392q0 71 51 119t122 48q71 0 122-48t51-119v-426h111v426Z" })), oe = cA(Ne), Re = (A, e) => /* @__PURE__ */ J.createElement("svg", { viewBox: "0 -960 960 960", ref: e, ...A }, /* @__PURE__ */ J.createElement("path", { d: "M125-165v-113h77v-273q0-93 55-167 56-75 146-100v-19q0-32 22-55 23-23 55-23t55 23q22 23 22 55v19q90 25 146 99 56 73 56 168v273h77v113H125ZM480-32q-38 0-66-27-27-27-27-66h186q0 39-27 66t-66 27Z" })), Ue = cA(Re), Fe = (A, e) => /* @__PURE__ */ J.createElement("svg", { viewBox: "0 -960 960 960", ref: e, ...A }, /* @__PURE__ */ J.createElement("path", { d: "M137-190v-113h78v-253q0-41 11-79 11-39 35-71l-9 150h10L17-802l74-74L873-92l-74 74-175-172H137Zm609-149L319-769q20-15 41-25t44-16v-28q0-32 22-54t54-22q32 0 55 22t22 54v28q85 23 137 94t52 160v217ZM481-47q-38 0-65-27t-27-65h184q0 38-27 65t-65 27Z" })), je = cA(Fe), ze = (A, e) => /* @__PURE__ */ J.createElement("svg", { xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 100 100", ref: e, ...A }, /* @__PURE__ */ J.createElement("g", { strokeLinecap: "round", strokeWidth: 10 }, /* @__PURE__ */ J.createElement("path", { d: "m19 19 61 61M80 19 19 80" }))), Ge = cA(ze), Ye = (A, e) => /* @__PURE__ */ J.createElement("svg", { viewBox: "0 -960 960 960", ref: e, ...A }, /* @__PURE__ */ J.createElement("path", { d: "M94-136v-255l351-93-351-92v-253l816 346L94-136Z" })), Xe = cA(Ye), We = (A, e) => /* @__PURE__ */ J.createElement("svg", { viewBox: "0 -960 960 960", ref: e, ...A }, /* @__PURE__ */ J.createElement("path", { d: "M480-372q-61 0-105-43-43-43-43-105v-240q0-61 43-105 43-43 105-43 61 0 105 43 43 44 43 105v240q0 62-43 105t-105 43ZM425-76v-122q-121-16-197-108t-76-214h109q0 91 64 154t155 63q91 0 155-63t64-154h109q0 122-77 214T534-198v122H425Z" })), Ke = cA(We), qe = (A, e) => /* @__PURE__ */ J.createElement("svg", { viewBox: "0 -960 960 960", ref: e, ...A }, /* @__PURE__ */ J.createElement("path", { d: "m763-343-80-79q8-20 13-44 5-23 5-54h109q0 49-11 95t-36 82ZM625-482 336-770v-2q3-56 45-96t101-40q62 0 105 43t43 105v249l-1 17q-1 7-4 12ZM427-76v-122q-120-16-196-108-77-92-77-214h109q0 91 64 154t155 63q43 0 80-14 37-15 67-41l78 79q-35 32-78 53t-93 28v122H427Zm362 27L41-796l75-75 748 748-75 74Z" })), Je = cA(qe), Ve = (A, e) => /* @__PURE__ */ J.createElement("svg", { viewBox: "0 -960 960 960", ref: e, ...A }, /* @__PURE__ */ J.createElement("path", { d: "M625-511q29 0 50-21t20-50q0-29-20-50-21-20-50-20t-50 20-20 50q0 29 20 50 21 21 50 21Zm-290 0q29 0 50-21t20-49q0-30-20-50t-50-21q-29 0-50 20-20 21-20 50 0 30 20 50 21 21 50 21Zm145 268q75 0 137-43t85-115H258q24 72 85 115 62 43 137 43Zm0 198q-90 0-170-34-79-34-138-93T79-310q-34-80-34-170t34-170q34-79 93-138t138-93q80-34 170-34t170 34q79 34 138 93t93 138q34 80 34 170t-34 170q-34 79-93 138T650-79q-80 34-170 34Zm0-435Zm0 322q134 0 228-94t94-228q0-134-94-228t-228-94q-134 0-228 94t-94 228q0 134 94 228t228 94Z" })), ne = cA(Ve), Ze = (A, e) => /* @__PURE__ */ J.createElement("svg", { viewBox: "0 -960 960 960", ref: e, ...A }, /* @__PURE__ */ J.createElement("path", { d: "M553-64v-108q107-27 173-113 67-87 67-197 0-109-67-195T553-789v-108q152 26 249 144 97 117 97 271 0 155-97 273T553-64ZM61-335v-290h176l236-237v764L237-335H61Zm492 28v-346q51 23 82 70t31 103-31 103q-31 47-82 70Z" })), _e = cA(Ze), $e = (A, e) => /* @__PURE__ */ J.createElement("svg", { viewBox: "0 -960 960 960", ref: e, ...A }, /* @__PURE__ */ J.createElement("path", { d: "M807-15 700-123q-33 21-71 37-38 15-80 22v-108q20-4 38-12l35-17-153-154v257L234-335H57v-290h144L10-818l74-75L882-90l-75 75Zm37-264-80-79q13-30 19-60 7-31 7-64 0-109-68-195-67-86-173-112v-108q152 26 249 144 98 117 98 271 0 55-14 107-13 51-38 96ZM661-462 549-575v-78q51 23 83 70 31 47 31 103l-1 9-1 9ZM469-655 366-759l103-103v207Z" })), At = cA($e), et = "data:audio/wav;base64,", ce = {
  general: {
    primaryColor: "#42b0c5",
    secondaryColor: "#491d8d",
    fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', 'Roboto', 'Oxygen', 'Ubuntu', 'Cantarell', 'Fira Sans', 'Droid Sans', 'Helvetica Neue', sans-serif",
    showHeader: !0,
    showFooter: !0,
    showInputRow: !0,
    actionDisabledIcon: Oe,
    embedded: !1,
    flowStartTrigger: "ON_LOAD"
  },
  tooltip: {
    mode: "CLOSE",
    text: "Talk to me! 😊"
  },
  chatButton: {
    icon: te
  },
  header: {
    title: /* @__PURE__ */ l("div", { style: { cursor: "pointer", margin: 0, fontSize: 20, fontWeight: "bold" }, onClick: () => window.open("https://github.com/tjtanjin/"), children: "Tan Jin" }),
    showAvatar: !0,
    avatar: ee,
    buttons: [V.NOTIFICATION_BUTTON, V.AUDIO_BUTTON, V.CLOSE_CHAT_BUTTON],
    closeChatIcon: Ge
  },
  notification: {
    disabled: !1,
    defaultToggledOn: !0,
    volume: 0.2,
    icon: Ue,
    iconDisabled: je,
    sound: et,
    showCount: !0
  },
  audio: {
    disabled: !0,
    defaultToggledOn: !1,
    language: "en-US",
    voiceNames: ["Microsoft David - English (United States)", "Alex (English - United States)"],
    rate: 1,
    volume: 1,
    icon: _e,
    iconDisabled: At
  },
  chatHistory: {
    disabled: !1,
    maxEntries: 30,
    storageKey: "rcb-history",
    storageType: "LOCAL_STORAGE",
    viewChatHistoryButtonText: "Load Chat History ⟳",
    chatHistoryLineBreakText: "----- Previous Chat History -----",
    autoLoad: !1
  },
  chatInput: {
    disabled: !1,
    allowNewline: !1,
    enabledPlaceholderText: "Type your message...",
    disabledPlaceholderText: "",
    showCharacterCount: !1,
    characterLimit: -1,
    botDelay: 1e3,
    sendButtonIcon: Xe,
    blockSpam: !0,
    sendOptionOutput: !0,
    sendCheckboxOutput: !0,
    buttons: [V.VOICE_MESSAGE_BUTTON, V.SEND_MESSAGE_BUTTON]
  },
  chatWindow: {
    showScrollbar: !1,
    showTypingIndicator: !0,
    autoJumpToBottom: !1,
    showMessagePrompt: !0,
    messagePromptText: "New Messages ↓",
    messagePromptOffset: 30,
    defaultOpen: !1
  },
  sensitiveInput: {
    maskInTextArea: !0,
    maskInUserBubble: !0,
    asterisksCount: 10,
    hideInUserBubble: !1
  },
  userBubble: {
    animate: !0,
    showAvatar: !1,
    avatar: xe,
    simulateStream: !1,
    streamSpeed: 30
  },
  botBubble: {
    animate: !0,
    showAvatar: !1,
    avatar: ee,
    simulateStream: !1,
    streamSpeed: 30
  },
  voice: {
    disabled: !0,
    defaultToggledOn: !1,
    language: "en-US",
    timeoutPeriod: 1e4,
    autoSendDisabled: !1,
    autoSendPeriod: 1e3,
    sendAsAudio: !1,
    icon: Ke,
    iconDisabled: Je
  },
  footer: {
    text: /* @__PURE__ */ W(
      "div",
      {
        style: { cursor: "pointer", display: "flex", flexDirection: "row", alignItems: "center", columnGap: 3 },
        onClick: () => window.open("https://react-chatbotify.com"),
        children: [
          /* @__PURE__ */ l("span", { children: "Powered By " }, 0),
          /* @__PURE__ */ l(
            "div",
            {
              style: {
                borderRadius: "50%",
                width: 14,
                height: 14,
                display: "flex",
                justifyContent: "center",
                alignItems: "center",
                background: "linear-gradient(to right, #42b0c5, #491d8d)"
              },
              children: /* @__PURE__ */ l(te, { style: { width: "80%", height: "80%", fill: "#fff" } })
            },
            1
          ),
          /* @__PURE__ */ l("span", { style: { fontWeight: "bold" }, children: " React ChatBotify" }, 2)
        ]
      }
    ),
    buttons: [V.FILE_ATTACHMENT_BUTTON, V.EMOJI_PICKER_BUTTON]
  },
  fileAttachment: {
    disabled: !1,
    multiple: !0,
    accept: ".png",
    icon: oe,
    iconDisabled: oe,
    sendFileName: !0,
    showMediaDisplay: !1
  },
  emoji: {
    disabled: !1,
    icon: ne,
    iconDisabled: ne,
    list: ["😀", "😃", "😄", "😅", "😊", "😌", "😇", "🙃", "🤣", "😍", "🥰", "🥳", "🎉", "🎈", "🚀", "⭐️"]
  },
  toast: {
    maxCount: 3,
    forbidOnMax: !1,
    dismissOnClick: !0
  },
  event: {
    rcbPreInjectMessage: !1,
    rcbPostInjectMessage: !1,
    rcbStartSimulateStreamMessage: !1,
    rcbStopSimulateStreamMessage: !1,
    rcbStartStreamMessage: !1,
    rcbChunkStreamMessage: !1,
    rcbStopStreamMessage: !1,
    rcbRemoveMessage: !1,
    rcbLoadChatHistory: !1,
    rcbToggleChatWindow: !1,
    rcbStartSpeakAudio: !1,
    rcbToggleAudio: !1,
    rcbToggleNotifications: !1,
    rcbToggleVoice: !1,
    rcbChangePath: !1,
    rcbShowToast: !1,
    rcbDismissToast: !1,
    rcbUserSubmitText: !1,
    rcbUserUploadFile: !1,
    rcbTextAreaChangeValue: !1,
    rcbPreLoadChatBot: !1,
    rcbPostLoadChatBot: !1,
    rcbPreProcessBlock: !1,
    rcbPostProcessBlock: !1
  },
  ariaLabel: {
    chatButton: "open chat",
    audioButton: "toggle audio",
    closeChatButton: "close chat",
    emojiButton: "emoji picker",
    fileAttachmentButton: "upload file",
    notificationButton: "toggle notifications",
    sendButton: "send message",
    voiceButton: "toggle voice",
    inputTextArea: "input text area"
  },
  device: {
    desktopEnabled: !0,
    mobileEnabled: !0,
    applyMobileOptimizations: !0
  }
}, ie = BA({ settings: {}, setSettings: () => null }), N = () => vA(ie), tt = ({
  children: A,
  settings: e = ce,
  setSettings: o
}) => /* @__PURE__ */ l(ie.Provider, { value: { settings: e, setSettings: o }, children: A }), le = {
  tooltipStyle: {},
  notificationBadgeStyle: {},
  chatWindowStyle: {},
  headerStyle: {},
  bodyStyle: {},
  chatInputContainerStyle: {},
  chatInputAreaStyle: {},
  chatInputAreaFocusedStyle: {},
  chatInputAreaDisabledStyle: {},
  userBubbleStyle: {},
  botBubbleStyle: {},
  botOptionStyle: {},
  botOptionHoveredStyle: {},
  botCheckboxRowStyle: {},
  botCheckboxNextStyle: {},
  botCheckMarkStyle: {},
  botCheckMarkSelectedStyle: {},
  characterLimitStyle: {},
  characterLimitReachedStyle: {},
  chatHistoryLineBreakStyle: {},
  chatMessagePromptStyle: {},
  chatMessagePromptHoveredStyle: {},
  footerStyle: {},
  loadingSpinnerStyle: {},
  mediaDisplayContainerStyle: {},
  chatButtonStyle: {},
  chatHistoryButtonStyle: {},
  chatHistoryButtonHoveredStyle: {},
  sendButtonStyle: {},
  sendButtonHoveredStyle: {},
  sendButtonDisabledStyle: {},
  audioButtonStyle: {},
  audioButtonDisabledStyle: {},
  closeChatButtonStyle: {},
  emojiButtonStyle: {},
  emojiButtonDisabledStyle: {},
  fileAttachmentButtonStyle: {},
  fileAttachmentButtonDisabledStyle: {},
  notificationButtonStyle: {},
  notificationButtonDisabledStyle: {},
  voiceButtonStyle: {},
  voiceButtonDisabledStyle: {},
  chatIconStyle: {},
  audioIconStyle: {},
  audioIconDisabledStyle: {},
  closeChatIconStyle: {},
  emojiIconStyle: {},
  emojiIconDisabledStyle: {},
  fileAttachmentIconStyle: {},
  fileAttachmentIconDisabledStyle: {},
  notificationIconStyle: {},
  notificationIconDisabledStyle: {},
  voiceIconStyle: {},
  voiceIconDisabledStyle: {},
  sendIconStyle: {},
  sendIconDisabledStyle: {},
  sendIconHoveredStyle: {},
  rcbTypingIndicatorContainerStyle: {},
  rcbTypingIndicatorDotStyle: {},
  toastPromptContainerStyle: {},
  toastPromptStyle: {},
  toastPromptHoveredStyle: {}
}, ue = BA({ styles: {}, setStyles: () => null }), X = () => vA(ue), ot = ({
  children: A,
  styles: e = le,
  setStyles: o
}) => /* @__PURE__ */ l(ue.Provider, { value: { styles: e, setStyles: o }, children: A }), nt = ({ buttons: A }) => {
  var n, s, r, a, c;
  const { settings: e } = N(), { styles: o } = X(), t = {
    backgroundImage: `linear-gradient(to right, ${(n = e.general) == null ? void 0 : n.secondaryColor},
			${(s = e.general) == null ? void 0 : s.primaryColor})`,
    ...o.headerStyle
  };
  return /* @__PURE__ */ W("div", { style: t, className: "rcb-chat-header-container", children: [
    /* @__PURE__ */ W("div", { className: "rcb-chat-header", children: [
      ((r = e.header) == null ? void 0 : r.showAvatar) && /* @__PURE__ */ l(
        "div",
        {
          style: { backgroundImage: `url("${(a = e.header) == null ? void 0 : a.avatar}")` },
          className: "rcb-bot-avatar"
        }
      ),
      (c = e.header) == null ? void 0 : c.title
    ] }),
    /* @__PURE__ */ l("div", { className: "rcb-chat-header", children: A == null ? void 0 : A.map(
      (i, u) => /* @__PURE__ */ l(UA, { children: i }, u)
    ) })
  ] });
}, fe = BA({}), $ = () => vA(fe), st = ({
  children: A,
  botIdRef: e,
  flowRef: o
}) => {
  const t = eA(null), n = eA(""), s = eA(/* @__PURE__ */ new Map()), r = eA(null), a = eA(""), c = eA(!1), i = eA(null), u = eA(null), f = eA(null);
  return /* @__PURE__ */ l(fe.Provider, { value: {
    botIdRef: e,
    flowRef: o,
    inputRef: t,
    streamMessageMap: s,
    chatBodyRef: r,
    paramsInputRef: a,
    keepVoiceOnRef: c,
    audioContextRef: i,
    audioBufferRef: u,
    gainNodeRef: f,
    prevInputRef: n
  }, children: A });
}, rA = (A) => {
  const [e, o] = z(A), t = eA(A);
  return [e, (s) => {
    const r = typeof s == "function" ? s(t.current) : s;
    t.current = r, o(r);
  }, t];
}, de = BA({}), AA = () => vA(de), rt = ({
  children: A,
  settings: e
}) => {
  var qA, JA, VA, ZA, _A, $A;
  const [o, t, n] = rA(!1), [s, r, a] = rA(
    ((qA = e == null ? void 0 : e.chatWindow) == null ? void 0 : qA.defaultOpen) ?? !1
  ), [c, i, u] = rA(
    ((JA = e == null ? void 0 : e.audio) == null ? void 0 : JA.defaultToggledOn) ?? !1
  ), [f, d, w] = rA(
    ((VA = e == null ? void 0 : e.voice) == null ? void 0 : VA.defaultToggledOn) ?? !1
  ), [
    g,
    B,
    D
  ] = rA(
    ((ZA = e == null ? void 0 : e.notification) == null ? void 0 : ZA.defaultToggledOn) ?? !0
  ), [y, P] = z(!1), [Q, h] = z(!1), [v, C, p] = rA(!1), [E, I, m] = rA(!0), [
    S,
    O,
    k
  ] = rA(!1), [b, U] = z(!1), [R, F] = z(!1), [q, K] = z(0), [L, x] = z(0), [G, H] = z(!1), [j, Y] = z(null), [_, tA] = z(((_A = window.visualViewport) == null ? void 0 : _A.height) ?? window.innerHeight), [nA, PA] = z((($A = window.visualViewport) == null ? void 0 : $A.width) ?? window.innerWidth);
  return /* @__PURE__ */ l(de.Provider, { value: {
    isBotTyping: o,
    setSyncedIsBotTyping: t,
    syncedIsBotTypingRef: n,
    isChatWindowOpen: s,
    setSyncedIsChatWindowOpen: r,
    syncedIsChatWindowOpenRef: a,
    audioToggledOn: c,
    setSyncedAudioToggledOn: i,
    syncedAudioToggledOnRef: u,
    voiceToggledOn: f,
    setSyncedVoiceToggledOn: d,
    syncedVoiceToggledOnRef: w,
    notificationsToggledOn: g,
    setSyncedNotificationsToggledOn: B,
    syncedNotificationsToggledOnRef: D,
    isLoadingChatHistory: y,
    setIsLoadingChatHistory: P,
    hasChatHistoryLoaded: Q,
    setHasChatHistoryLoaded: h,
    isScrolling: v,
    setSyncedIsScrolling: C,
    syncedIsScrollingRef: p,
    textAreaDisabled: E,
    setSyncedTextAreaDisabled: I,
    syncedTextAreaDisabledRef: m,
    textAreaSensitiveMode: S,
    setSyncedTextAreaSensitiveMode: O,
    syncedTextAreaSensitiveModeRef: k,
    hasInteractedPage: b,
    setHasInteractedPage: U,
    hasFlowStarted: R,
    setHasFlowStarted: F,
    unreadCount: q,
    setUnreadCount: K,
    inputLength: L,
    setInputLength: x,
    blockAllowsAttachment: G,
    setBlockAllowsAttachment: H,
    timeoutId: j,
    setTimeoutId: Y,
    viewportHeight: _,
    setViewportHeight: tA,
    viewportWidth: nA,
    setViewportWidth: PA
  }, children: A });
}, M = {
  // audio
  START_SPEAK_AUDIO: "rcb-start-speak-audio",
  TOGGLE_AUDIO: "rcb-toggle-audio",
  // notifications:
  TOGGLE_NOTIFICATIONS: "rcb-toggle-notifications",
  // voice
  TOGGLE_VOICE: "rcb-toggle-voice",
  // chat window
  TOGGLE_CHAT_WINDOW: "rcb-toggle-chat-window",
  // messages:
  PRE_INJECT_MESSAGE: "rcb-pre-inject-message",
  POST_INJECT_MESSAGE: "rcb-post-inject-message",
  START_SIMULATE_STREAM_MESSAGE: "rcb-start-simulate-stream-message",
  STOP_SIMULATE_STREAM_MESSAGE: "rcb-stop-simulate-stream-message",
  START_STREAM_MESSAGE: "rcb-start-stream-message",
  CHUNK_STREAM_MESSAGE: "rcb-chunk-stream-message",
  STOP_STREAM_MESSAGE: "rcb-stop-stream-message",
  REMOVE_MESSAGE: "rcb-remove-message",
  // chat history
  LOAD_CHAT_HISTORY: "rcb-load-chat-history",
  // path
  CHANGE_PATH: "rcb-change-path",
  // toast
  SHOW_TOAST: "rcb-show-toast",
  DISMISS_TOAST: "rcb-dismiss-toast",
  // user input submission
  USER_SUBMIT_TEXT: "rcb-user-submit-text",
  USER_UPLOAD_FILE: "rcb-user-upload-file",
  // text area value change
  TEXT_AREA_CHANGE_VALUE: "rcb-text-area-change-value",
  // chatbot loading
  PRE_LOAD_CHATBOT: "rcb-pre-load-chatbot",
  POST_LOAD_CHATBOT: "rcb-post-load-chatbot",
  // block processing
  PRE_PROCESS_BLOCK: "rcb-pre-process-block",
  POST_PROCESS_BLOCK: "rcb-post-process-block"
}, at = {
  [M.START_SPEAK_AUDIO]: !0,
  [M.TOGGLE_AUDIO]: !0,
  [M.TOGGLE_VOICE]: !0,
  [M.TOGGLE_NOTIFICATIONS]: !0,
  [M.TOGGLE_CHAT_WINDOW]: !0,
  [M.PRE_INJECT_MESSAGE]: !0,
  [M.POST_INJECT_MESSAGE]: !1,
  [M.PRE_LOAD_CHATBOT]: !0,
  [M.POST_LOAD_CHATBOT]: !1,
  [M.START_SIMULATE_STREAM_MESSAGE]: !0,
  [M.STOP_SIMULATE_STREAM_MESSAGE]: !1,
  [M.START_STREAM_MESSAGE]: !0,
  [M.CHUNK_STREAM_MESSAGE]: !0,
  [M.STOP_STREAM_MESSAGE]: !0,
  [M.LOAD_CHAT_HISTORY]: !0,
  [M.CHANGE_PATH]: !0,
  [M.SHOW_TOAST]: !0,
  [M.DISMISS_TOAST]: !0,
  [M.USER_SUBMIT_TEXT]: !0,
  [M.USER_UPLOAD_FILE]: !0,
  [M.TEXT_AREA_CHANGE_VALUE]: !0,
  [M.PRE_PROCESS_BLOCK]: !0,
  [M.POST_PROCESS_BLOCK]: !0
}, NA = async (A, e, o) => {
  const t = new CustomEvent(A, {
    detail: e,
    cancelable: at[A]
  });
  return t.data = o, t.promises = [], window.dispatchEvent(t), await Promise.all(t.promises), t;
}, we = BA({
  paths: [],
  setSyncedPaths: () => {
  },
  syncedPathsRef: { current: [] }
}), bA = () => vA(we), ct = ({ children: A }) => {
  const [e, o, t] = rA([]);
  return /* @__PURE__ */ l(we.Provider, { value: { paths: e, setSyncedPaths: o, syncedPathsRef: t }, children: A });
}, iA = () => {
  const { botIdRef: A } = $(), { syncedPathsRef: e } = bA(), o = T(() => e.current.length > 0 ? e.current.at(-1) ?? null : null, []), t = T(() => e.current.length > 1 ? e.current.at(-2) ?? null : null, []);
  return { callRcbEvent: T(async (s, r) => {
    const a = { botId: A.current, currPath: o(), prevPath: t() };
    return await NA(s, a, r);
  }, [A]) };
}, sA = () => {
  const { settings: A } = N(), {
    isChatWindowOpen: e,
    setSyncedIsChatWindowOpen: o,
    viewportHeight: t,
    setViewportHeight: n,
    viewportWidth: s,
    setViewportWidth: r,
    setUnreadCount: a,
    setSyncedIsBotTyping: c,
    setSyncedIsScrolling: i,
    syncedIsBotTypingRef: u,
    syncedIsChatWindowOpenRef: f
  } = AA(), { chatBodyRef: d } = $(), { callRcbEvent: w } = iA(), g = T(async (P) => {
    var Q;
    P !== f.current && ((Q = A.event) != null && Q.rcbToggleChatWindow && (await w(
      M.TOGGLE_CHAT_WINDOW,
      {
        currState: f.current,
        newState: !f.current
      }
    )).defaultPrevented || o((h) => (h || a(0), !h)));
  }, [f]), B = T(async (P) => {
    P !== u.current && c((Q) => !Q);
  }, [u]), D = T((P, Q, h, v) => (P /= v / 2, P < 1 ? h / 2 * P * P + Q : (P--, -h / 2 * (P * (P - 2) - 1) + Q)), []), y = T((P = 0) => {
    if (!d.current)
      return;
    const Q = d.current.scrollHeight - d.current.clientHeight;
    if (P <= 0) {
      d.current.scrollTop = Q, i(!1);
      return;
    }
    const h = d.current.scrollTop, v = Q - h, C = 20;
    let p = 0;
    const E = () => {
      if (!d.current)
        return;
      p += C;
      const I = D(p, h, v, P);
      d.current.scrollTop = I, p < P ? requestAnimationFrame(E) : i(!1);
    };
    E();
  }, [d]);
  return {
    isChatWindowOpen: e,
    setSyncedIsChatWindowOpen: o,
    toggleChatWindow: g,
    viewportHeight: t,
    setViewportHeight: n,
    viewportWidth: s,
    setViewportWidth: r,
    toggleIsBotTyping: B,
    scrollToBottom: y
  };
}, it = () => {
  var d, w, g;
  const { settings: A } = N(), { styles: e } = X(), { unreadCount: o, isScrolling: t } = AA(), { chatBodyRef: n } = $(), { scrollToBottom: s } = sA(), [r, a] = z(!1), c = {
    color: (d = A.general) == null ? void 0 : d.primaryColor,
    borderColor: (w = A.general) == null ? void 0 : w.primaryColor,
    ...e.chatMessagePromptHoveredStyle
  }, i = () => {
    a(!0);
  }, u = () => {
    a(!1);
  };
  return /* @__PURE__ */ l("div", { className: `rcb-message-prompt-container ${(() => {
    var D;
    return n.current && ((D = A.chatWindow) == null ? void 0 : D.showMessagePrompt) && t && o > 0 ? "visible" : "hidden";
  })()}`, children: /* @__PURE__ */ l(
    "div",
    {
      onMouseEnter: i,
      onMouseLeave: u,
      style: r ? c : { ...e.chatMessagePromptStyle },
      onMouseDown: (B) => {
        B.preventDefault(), s(600);
      },
      className: "rcb-message-prompt-text",
      children: (g = A.chatWindow) == null ? void 0 : g.messagePromptText
    }
  ) });
}, ge = BA({
  messages: [],
  setSyncedMessages: () => {
  },
  syncedMessagesRef: { current: [] }
}), FA = () => vA(ge), lt = ({ children: A }) => {
  const [e, o, t] = rA([]);
  return /* @__PURE__ */ l(ge.Provider, { value: { messages: e, setSyncedMessages: o, syncedMessagesRef: t }, children: A });
}, ut = ({
  message: A,
  isNewSender: e
}) => {
  var f, d, w, g, B, D;
  const { settings: o } = N(), { styles: t } = X(), n = typeof A.content == "string", s = A.content, r = A.contentWrapper ? /* @__PURE__ */ l(A.contentWrapper, { children: s }) : s, a = {
    backgroundColor: (f = o.general) == null ? void 0 : f.primaryColor,
    color: "#fff",
    maxWidth: (d = o.userBubble) != null && d.showAvatar ? "65%" : "70%",
    ...t.userBubbleStyle
  }, c = (w = o.userBubble) != null && w.animate ? "rcb-user-message-entry" : "", i = ((g = o.userBubble) == null ? void 0 : g.showAvatar) && e, u = `rcb-user-message${!e && ((B = o.userBubble) != null && B.showAvatar) ? " rcb-user-message-offset" : ""}`;
  return /* @__PURE__ */ W("div", { className: "rcb-user-message-container", children: [
    n ? /* @__PURE__ */ l("div", { style: a, className: `${u} ${c}`, children: r }) : /* @__PURE__ */ l(oA, { children: r }),
    i && /* @__PURE__ */ l(
      "div",
      {
        style: { backgroundImage: `url("${(D = o.userBubble) == null ? void 0 : D.avatar}")` },
        className: "rcb-message-user-avatar"
      }
    )
  ] });
}, ft = ({
  message: A,
  isNewSender: e
}) => {
  var f, d, w, g, B, D;
  const { settings: o } = N(), { styles: t } = X(), n = typeof A.content == "string", s = A.content, r = A.contentWrapper ? /* @__PURE__ */ l(A.contentWrapper, { children: s }) : s, a = {
    backgroundColor: (f = o.general) == null ? void 0 : f.secondaryColor,
    color: "#fff",
    maxWidth: (d = o.botBubble) != null && d.showAvatar ? "65%" : "70%",
    ...t.botBubbleStyle
  }, c = (w = o.botBubble) != null && w.animate ? "rcb-bot-message-entry" : "", i = ((g = o.botBubble) == null ? void 0 : g.showAvatar) && e, u = `rcb-bot-message${!e && ((B = o.botBubble) != null && B.showAvatar) ? " rcb-bot-message-offset" : ""}`;
  return /* @__PURE__ */ W("div", { className: "rcb-bot-message-container", children: [
    i && /* @__PURE__ */ l(
      "div",
      {
        style: { backgroundImage: `url("${(D = o.botBubble) == null ? void 0 : D.avatar}")` },
        className: "rcb-message-bot-avatar"
      }
    ),
    n ? /* @__PURE__ */ l("div", { style: a, className: `${u} ${c}`, children: r }) : /* @__PURE__ */ l(oA, { children: r })
  ] });
}, dt = () => {
  var t, n, s;
  const { settings: A } = N(), { styles: e } = X(), o = (t = A.botBubble) != null && t.animate ? "rcb-bot-message-entry" : "";
  return /* @__PURE__ */ W("div", { className: "rcb-bot-message-container", children: [
    ((n = A.botBubble) == null ? void 0 : n.showAvatar) && /* @__PURE__ */ l(
      "div",
      {
        style: { backgroundImage: `url("${(s = A.botBubble) == null ? void 0 : s.avatar}")` },
        className: "rcb-message-bot-avatar"
      }
    ),
    /* @__PURE__ */ l(
      "div",
      {
        onMouseDown: (r) => {
          r.preventDefault();
        },
        className: `rcb-bot-message ${o}`,
        children: /* @__PURE__ */ W("div", { className: "rcb-typing-indicator", style: { ...e == null ? void 0 : e.rcbTypingIndicatorContainerStyle }, children: [
          /* @__PURE__ */ l("span", { className: "rcb-dot", style: { ...e == null ? void 0 : e.rcbTypingIndicatorDotStyle } }),
          /* @__PURE__ */ l("span", { className: "rcb-dot", style: { ...e == null ? void 0 : e.rcbTypingIndicatorDotStyle } }),
          /* @__PURE__ */ l("span", { className: "rcb-dot", style: { ...e == null ? void 0 : e.rcbTypingIndicatorDotStyle } })
        ] })
      }
    )
  ] });
}, wt = () => {
  var i, u, f;
  const { settings: A } = N(), { styles: e } = X(), { messages: o } = FA(), { scrollToBottom: t } = sA(), {
    isBotTyping: n,
    syncedIsScrollingRef: s
  } = AA(), { chatBodyRef: r } = $(), a = {
    ...e == null ? void 0 : e.bodyStyle,
    scrollbarWidth: (i = A.chatWindow) != null && i.showScrollbar ? "auto" : "none"
  };
  Z(() => {
    s.current || t();
  }, [(u = r.current) == null ? void 0 : u.scrollHeight]);
  const c = (d) => d === 0 ? !0 : o[d].sender !== o[d - 1].sender;
  return /* @__PURE__ */ W(
    "div",
    {
      style: a,
      className: "rcb-chat-body-container",
      ref: r,
      children: [
        o.map((d, w) => {
          const g = c(w);
          return d.sender.toUpperCase() === "USER" ? /* @__PURE__ */ l(ut, { message: d, isNewSender: g }, w) : d.sender.toUpperCase() === "BOT" ? /* @__PURE__ */ l(ft, { message: d, isNewSender: g }, w) : /* @__PURE__ */ l("div", { children: d.content }, w);
        }),
        n && ((f = A.chatWindow) == null ? void 0 : f.showTypingIndicator) && /* @__PURE__ */ l(dt, {}),
        /* @__PURE__ */ l(it, {})
      ]
    }
  );
}, gt = ({
  checkboxes: A,
  checkedItems: e,
  path: o
}) => {
  var D, y, P, Q, h, v, C, p, E, I, m;
  const { settings: t } = N(), { styles: n } = X(), { paths: s } = bA(), { handleSubmitText: r } = yA(), [a, c] = z(/* @__PURE__ */ new Set()), [i, u] = z(!1), f = {
    cursor: i ? `url("${(D = t.general) == null ? void 0 : D.actionDisabledIcon}"), auto` : "pointer",
    color: (y = t.general) == null ? void 0 : y.primaryColor,
    borderColor: (P = t.general) == null ? void 0 : P.primaryColor,
    ...n.botCheckboxRowStyle
  }, d = {
    cursor: i || a.size < A.min ? `url("${(Q = t.general) == null ? void 0 : Q.actionDisabledIcon}"), auto` : "pointer",
    color: (h = t.general) == null ? void 0 : h.primaryColor,
    borderColor: (v = t.general) == null ? void 0 : v.primaryColor,
    ...n.botCheckboxNextStyle
  }, w = {
    cursor: i ? `url("${(C = t.general) == null ? void 0 : C.actionDisabledIcon}"), auto` : "pointer",
    color: "transparent",
    ...n.botCheckMarkStyle
  }, g = {
    cursor: i ? `url("${(p = t.general) == null ? void 0 : p.actionDisabledIcon}"), auto` : "pointer",
    color: "#fff",
    borderColor: (E = t.general) == null ? void 0 : E.primaryColor,
    backgroundColor: (I = t.general) == null ? void 0 : I.primaryColor,
    ...n.botCheckMarkSelectedStyle
  };
  Z(() => {
    s.length > 0 && s[s.length - 1] !== o && u(!A.reusable);
  }, [s]);
  const B = (S) => {
    i || c((O) => {
      const k = new Set(O);
      if (k.has(S))
        e.delete(S), k.delete(S);
      else {
        if (a.size == A.max)
          return O;
        e.add(S), k.add(S);
      }
      return k;
    });
  };
  return /* @__PURE__ */ W("div", { className: `rcb-checkbox-container ${(m = t.botBubble) != null && m.showAvatar ? "rcb-checkbox-offset" : ""}`, children: [
    A.items.map((S) => /* @__PURE__ */ l(
      "div",
      {
        onMouseDown: (O) => {
          O.preventDefault(), B(S);
        },
        style: f,
        className: "rcb-checkbox-row-container",
        children: /* @__PURE__ */ W("div", { className: "rcb-checkbox-row", children: [
          /* @__PURE__ */ l(
            "div",
            {
              style: a.has(S) ? g : w,
              className: "rcb-checkbox-mark"
            }
          ),
          /* @__PURE__ */ l("div", { className: "rcb-checkbox-label", children: S })
        ] })
      },
      S
    )),
    /* @__PURE__ */ l(
      "div",
      {
        style: d,
        className: "rcb-checkbox-next-button",
        onMouseDown: (S) => {
          var b;
          if (S.preventDefault(), i || a.size < A.min)
            return;
          const O = Array.from(e).join(", ");
          u(!A.reusable);
          let k;
          A.sendOutput != null ? k = A.sendOutput : k = ((b = t.chatInput) == null ? void 0 : b.sendCheckboxOutput) ?? !0, r(O, k);
        }
      }
    )
  ] });
}, Bt = async (A, e) => {
  const o = A.checkboxes;
  if (!o)
    return;
  let t;
  if (typeof o == "function" ? (t = o(e), t instanceof Promise && (t = await t)) : t = o, Array.isArray(t) && (t = { items: t }), !("items" in t) || t.items.length == 0)
    return;
  t.min == null && (t.min = 1), t.max == null && (t.max = t.items.length), t.min > t.max && (t.min = t.max), t.reusable == null && (t.reusable = !1);
  const n = /* @__PURE__ */ new Set(), s = e.currPath, r = /* @__PURE__ */ l(gt, { checkboxes: t, checkedItems: n, path: s });
  await e.injectMessage(r);
}, vt = async (A, e) => {
  const o = A.function;
  if (!o)
    return;
  const t = o(e);
  return t instanceof Promise ? await t : t;
}, Pt = async (A, e, o) => {
  const t = A.message;
  if (!t)
    return;
  if (typeof t == "string") {
    t.trim() !== "" && (o ? await e.simulateStreamMessage(t) : await e.injectMessage(t));
    return;
  }
  let n = t(e);
  n instanceof Promise && (n = await n), typeof n == "string" && n.trim() !== "" && (o ? await e.simulateStreamMessage(n) : await e.injectMessage(n));
}, ht = ({
  options: A,
  path: e
}) => {
  var g, B, D, y, P, Q;
  const { settings: o } = N(), { styles: t } = X(), { paths: n } = bA(), { handleSubmitText: s } = yA(), [r, a] = z([]), [c, i] = z(!1), u = {
    cursor: c ? `url("${(g = o.general) == null ? void 0 : g.actionDisabledIcon}"), auto` : "pointer",
    color: (B = o.general) == null ? void 0 : B.primaryColor,
    borderColor: (D = o.general) == null ? void 0 : D.primaryColor,
    backgroundColor: "#fff",
    ...t.botOptionStyle
  }, f = {
    color: "#fff",
    borderColor: (y = o.general) == null ? void 0 : y.primaryColor,
    backgroundColor: (P = o.general) == null ? void 0 : P.primaryColor,
    ...t.botOptionHoveredStyle
  };
  Z(() => {
    n.length > 0 && n[n.length - 1] !== e && i(!A.reusable);
  }, [n]);
  const d = (h) => {
    a((v) => {
      const C = [...v];
      return C[h] = !0, C;
    });
  }, w = (h) => {
    a((v) => {
      const C = [...v];
      return C[h] = !1, C;
    });
  };
  return /* @__PURE__ */ l("div", { className: `rcb-options-container ${(Q = o.botBubble) != null && Q.showAvatar ? "rcb-options-offset" : ""}`, children: A.items.map((h, v) => {
    const C = r[v] && !c;
    return /* @__PURE__ */ l(
      "div",
      {
        className: "rcb-options",
        style: C ? f : u,
        onMouseEnter: () => d(v),
        onMouseLeave: () => w(v),
        onMouseDown: (p) => {
          var I;
          if (p.preventDefault(), c)
            return;
          i(!A.reusable);
          let E;
          A.sendOutput != null ? E = A.sendOutput : E = ((I = o.chatInput) == null ? void 0 : I.sendOptionOutput) ?? !0, s(h, E);
        },
        children: h
      },
      h
    );
  }) });
}, Dt = async (A, e) => {
  const o = A.options;
  if (!o)
    return;
  let t;
  if (typeof o == "function" ? (t = o(e), t instanceof Promise && (t = await t)) : t = o, Array.isArray(t) && (t = { items: t }), !("items" in t) || t.items.length == 0)
    return;
  t.reusable == null && (t.reusable = !1);
  const n = e.currPath, s = /* @__PURE__ */ l(ht, { options: t, path: n });
  await e.injectMessage(s);
}, Et = async (A, e) => {
  const o = A.path;
  if (!o)
    return !1;
  if (typeof o == "string")
    return await e.goToPath(o), !0;
  let t = o(e);
  if (t instanceof Promise && (t = await t), !t)
    return !1;
  const n = t;
  return await e.goToPath(n), !0;
}, yt = async (A, e) => {
  const o = A.component;
  if (o) {
    if (typeof o == "function") {
      let t = o(e);
      if (t instanceof Promise && (t = await t), !t)
        return;
      await e.injectMessage(t);
      return;
    }
    await e.injectMessage(o);
  }
}, Ct = async (A, e, o, t) => {
  const n = A.transition;
  let s;
  if (typeof n == "function" ? (s = n(e), s instanceof Promise && (s = await s)) : s = n, typeof s == "number" && (s = { duration: s }), !s || s instanceof Promise || s.duration == null || typeof s.duration != "number")
    return;
  s.interruptable == null && (s.interruptable = !1);
  const r = setTimeout(async () => {
    const a = await t(A);
    a && await ve(a, e);
  }, s.duration);
  s.interruptable && o(r);
}, bt = async (A, e, o) => {
  const t = A.chatDisabled;
  if (t == null)
    return;
  let n;
  typeof t == "function" ? (n = t(e), n instanceof Promise && (n = await n)) : n = t, o(n);
}, Be = async (A, e, o) => {
  const t = A.isSensitive;
  if (!t) {
    o(!1);
    return;
  }
  let n;
  typeof t == "function" ? (n = t(e), n instanceof Promise && (n = await n)) : n = t, o(n);
}, Qt = async (A, e, o, t, n, s, r) => {
  if (!A)
    throw new Error("Block is not valid.");
  for (const a of Object.keys(A))
    switch (a) {
      case "message":
        await Pt(A, e, o);
        break;
      case "options":
        await Dt(A, e);
        break;
      case "checkboxes":
        await Bt(A, e);
        break;
      case "component":
        await yt(A, e);
        break;
      case "chatDisabled":
        await bt(A, e, t);
        break;
      case "isSensitive":
        await Be(A, e, n);
        break;
      case "transition":
        await Ct(A, e, s, r);
    }
}, ve = async (A, e) => {
  if (!A)
    throw new Error("Block is not valid.");
  const o = Object.keys(A);
  for (const t of o)
    t === "function" && await vt(A, e);
  o.includes("path") && await Et(A, e);
}, mt = () => {
  var o;
  const { settings: A } = N(), { styles: e } = X();
  return /* @__PURE__ */ l("div", { className: "rcb-line-break-container", children: /* @__PURE__ */ l(
    "div",
    {
      style: { ...e.chatHistoryLineBreakStyle },
      className: "rcb-line-break-text",
      "data-testid": "chat-history-line-break-text",
      children: (o = A.chatHistory) == null ? void 0 : o.chatHistoryLineBreakText
    }
  ) });
}, pt = () => {
  var t;
  const { settings: A } = N(), { styles: e } = X(), o = {
    borderTop: `4px solid ${(t = A.general) == null ? void 0 : t.primaryColor}`,
    ...e.loadingSpinnerStyle
  };
  return /* @__PURE__ */ l("div", { className: "rcb-spinner-container", children: /* @__PURE__ */ l("div", { style: o, className: "rcb-spinner" }) });
}, SA = () => {
  try {
    if (typeof crypto < "u" && typeof crypto.randomUUID == "function")
      return crypto.randomUUID();
    throw new Error("crypto.randomUUID not available");
  } catch (A) {
    if (typeof crypto < "u" && typeof crypto.getRandomValues == "function")
      return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, function(e) {
        const o = crypto.getRandomValues(new Uint8Array(1))[0] % 16;
        return (e === "x" ? o : o & 3 | 8).toString(16);
      });
    throw console.error(A), new Error("crypto.getRandomValues is also not available");
  }
}, wA = (A, e) => ({
  id: SA(),
  content: A,
  sender: e,
  type: typeof A == "string" ? "string" : "object",
  timestamp: (/* @__PURE__ */ new Date()).toUTCString(),
  tags: []
});
let dA, Pe = !1, IA = "rcb-history", mA = 30, he = !1, CA = [];
const HA = async (A) => {
  if (he || !dA)
    return;
  const e = [], o = Pe ? CA.length : 0;
  for (let n = A.length - 1; n >= o; n--) {
    const s = A[n];
    if (s.sender.toUpperCase() !== "SYSTEM" && (s.content !== "" && e.unshift(s), e.length === mA))
      break;
  }
  let t = e.map(Mt);
  if (t.length < mA) {
    const n = mA - t.length;
    t = [...CA.slice(-n), ...t];
  }
  De(t);
}, St = (A) => {
  if (A != null)
    try {
      return JSON.parse(A);
    } catch {
      return [];
    }
  return [];
}, jA = () => CA, It = (A) => {
  De(A), CA = A;
}, De = (A) => {
  dA && dA.setItem(IA, JSON.stringify(A));
}, Tt = () => {
  dA && dA.removeItem(IA);
}, RA = (A) => {
  var e, o, t, n, s;
  ((o = (e = A.chatHistory) == null ? void 0 : e.storageType) == null ? void 0 : o.toUpperCase()) === "SESSION_STORAGE" ? dA = sessionStorage : dA = localStorage, IA = (t = A.chatHistory) == null ? void 0 : t.storageKey, mA = (n = A.chatHistory) == null ? void 0 : n.maxEntries, he = (s = A.chatHistory) == null ? void 0 : s.disabled, CA = St(dA.getItem(IA));
}, Mt = (A) => ae(A.content) ? structuredClone({
  id: A.id,
  content: He.renderToString(A.content),
  type: A.type,
  sender: A.sender.toUpperCase(),
  timestamp: A.timestamp,
  tags: A.tags
}) : A, kt = (A, e, o, t, n, s, r, a, c) => {
  var i;
  if (Pe = !0, o != null)
    try {
      const u = wA(/* @__PURE__ */ l(pt, {}), "SYSTEM"), f = n.current.slice(1);
      t([u, ...f]);
      const d = o.map((w) => {
        if (w.type === "object") {
          const g = Ee(w.content, A, e);
          return { ...w, content: g };
        }
        return w;
      });
      setTimeout(() => {
        var B;
        const w = n.current.slice(1);
        let g = (B = A.chatHistory) != null && B.autoLoad ? wA(/* @__PURE__ */ l(oA, {}), "SYSTEM") : wA(/* @__PURE__ */ l(mt, {}), "SYSTEM");
        t([...d, g, ...w]), c(!0);
      }, 500), setTimeout(() => {
        if (!s.current)
          return;
        const { scrollHeight: w } = s.current, g = w - r;
        s.current.scrollTop += g, a(!1);
      }, 510);
    } catch {
      dA.removeItem((i = A.chatHistory) == null ? void 0 : i.storageKey);
    }
}, Ee = (A, e, o) => {
  const n = new DOMParser().parseFromString(A, "text/html");
  return Array.from(n.body.childNodes).map((a, c) => {
    var i;
    if (a.nodeType === Node.TEXT_NODE)
      return a.textContent;
    {
      const u = a.tagName.toLowerCase();
      let f = Array.from(a.attributes).reduce((w, g) => {
        const B = g.name.toLowerCase();
        if (B === "style") {
          const D = g.value.split(";").filter((P) => P.trim() !== ""), y = {};
          D.forEach((P) => {
            const [Q, h] = P.split(":").map((C) => C.trim()), v = Q.replace(/-([a-z])/g, (C, p) => p.toUpperCase());
            y[v] = h;
          }), w[B] = y;
        } else (u === "audio" || u === "video") && B === "controls" && g.value === "" ? w[B] = "true" : w[B] = g.value;
        return w;
      }, {});
      if (Object.prototype.hasOwnProperty.call(f, "class")) {
        const w = a.classList;
        f.className = w.toString(), delete f.class, (i = e.botBubble) != null && i.showAvatar && (f = Ht(w, f)), f = Ot(w, f, e, o), f = xt(w, f, e, o), f = Lt(w, f, e, o), f = Nt(w, f, e, o);
      }
      if ([
        "area",
        "base",
        "br",
        "col",
        "embed",
        "hr",
        "img",
        "input",
        "link",
        "meta",
        "source",
        "track",
        "wbr"
      ].includes(u))
        return Ae(u, { key: c, ...f });
      {
        const w = Ee(a.innerHTML, e, o);
        return Ae(u, { key: c, ...f }, ...w);
      }
    }
  });
}, Ht = (A, e) => ((A.contains("rcb-options-container") || A.contains("rcb-checkbox-container")) && (e.className = `${A.toString()} rcb-options-offset`), e), Ot = (A, e, o, t) => {
  var n, s, r, a, c;
  return A.contains("rcb-options") && (e.style = {
    ...e.style,
    color: ((n = t.botOptionStyle) == null ? void 0 : n.color) ?? ((s = o.general) == null ? void 0 : s.primaryColor),
    borderColor: ((r = t.botOptionStyle) == null ? void 0 : r.color) ?? ((a = o.general) == null ? void 0 : a.primaryColor),
    cursor: `url("${(c = o.general) == null ? void 0 : c.actionDisabledIcon}"), auto`,
    ...t.botOptionStyle
  }), e;
}, xt = (A, e, o, t) => {
  var n, s, r, a, c;
  return A.contains("rcb-checkbox-row-container") && (e.style = {
    ...e.style,
    color: ((n = t.botCheckboxRowStyle) == null ? void 0 : n.color) ?? ((s = o.general) == null ? void 0 : s.primaryColor),
    borderColor: ((r = t.botCheckboxRowStyle) == null ? void 0 : r.color) ?? ((a = o.general) == null ? void 0 : a.primaryColor),
    cursor: `url("${(c = o.general) == null ? void 0 : c.actionDisabledIcon}"), auto`,
    ...t.botCheckboxRowStyle
  }), e;
}, Lt = (A, e, o, t) => {
  var n, s, r, a, c;
  return A.contains("rcb-checkbox-next-button") && (e.style = {
    ...e.style,
    color: ((n = t.botCheckboxNextStyle) == null ? void 0 : n.color) ?? ((s = o.general) == null ? void 0 : s.primaryColor),
    borderColor: ((r = t.botCheckboxNextStyle) == null ? void 0 : r.color) ?? ((a = o.general) == null ? void 0 : a.primaryColor),
    cursor: `url("${(c = o.general) == null ? void 0 : c.actionDisabledIcon}"), auto`,
    ...t.botCheckboxNextStyle
  }), e;
}, Nt = (A, e, o, t) => {
  var n, s;
  return (A.contains("rcb-media-display-image-container") || A.contains("rcb-media-display-video-container")) && (e.style = {
    ...e.style,
    backgroundColor: (n = o.general) == null ? void 0 : n.primaryColor,
    maxWidth: (s = o.userBubble) != null && s.showAvatar ? "65%" : "70%",
    ...t.mediaDisplayContainerStyle
  }), e;
}, ye = (A) => {
  if (!A)
    return !1;
  const e = A.getBoundingClientRect(), o = window.innerHeight ?? document.documentElement.clientHeight, t = window.innerWidth ?? document.documentElement.clientWidth;
  return e.top >= 0 && e.left >= 0 && e.bottom <= o && e.right <= t;
}, zA = () => {
  const { settings: A } = N(), {
    notificationsToggledOn: e,
    setSyncedNotificationsToggledOn: o,
    hasInteractedPage: t,
    unreadCount: n,
    setUnreadCount: s,
    syncedNotificationsToggledOnRef: r
  } = AA(), { audioBufferRef: a, audioContextRef: c, gainNodeRef: i } = $(), { callRcbEvent: u } = iA(), f = T(async () => {
    var y, P;
    const g = (y = A.notification) == null ? void 0 : y.sound;
    c.current = new AudioContext();
    const B = c.current.createGain();
    B.gain.value = ((P = A.notification) == null ? void 0 : P.volume) ?? 0.2, i.current = B;
    let D;
    if (g != null && g.startsWith("data:audio")) {
      const Q = atob(g.split(",")[1]), h = new ArrayBuffer(Q.length), v = new Uint8Array(h);
      for (let C = 0; C < Q.length; C++)
        v[C] = Q.charCodeAt(C);
      D = h;
    } else
      D = await (await fetch(g)).arrayBuffer();
    a.current = await c.current.decodeAudioData(D);
  }, [A.notification]), d = T(() => {
    var B;
    if ((B = A.notification) != null && B.disabled || !r.current || !t || !c.current || !a.current)
      return;
    const g = c.current.createBufferSource();
    g.buffer = a.current, g.connect(i.current).connect(c.current.destination), g.start();
  }, [
    A.notification,
    r,
    t,
    c,
    a,
    i
  ]), w = T(async (g) => {
    var B;
    g !== r.current && ((B = A.event) != null && B.rcbToggleNotifications && (await u(
      M.TOGGLE_NOTIFICATIONS,
      {
        currState: r.current,
        newState: !r.current
      }
    )).defaultPrevented || o((D) => !D));
  }, [r]);
  return {
    unreadCount: n,
    setUnreadCount: s,
    notificationsToggledOn: e,
    toggleNotifications: w,
    playNotificationSound: d,
    setUpNotifications: f
  };
}, Rt = (A, e, o, t, n) => {
  if (!window.SpeechSynthesisUtterance) {
    console.info("Speech Synthesis API is not supported in this environment.");
    return;
  }
  const s = new window.SpeechSynthesisUtterance();
  s.text = A, s.lang = e, s.rate = t, s.volume = n;
  let r = !1;
  for (const a of o)
    if (window.speechSynthesis.getVoices().find((c) => {
      if (c.name === a) {
        s.voice = c, window.speechSynthesis.speak(s), r = !0;
        return;
      }
    }), r)
      break;
  r || window.speechSynthesis.speak(s);
}, Ut = (A, e) => {
  var o, t, n, s;
  Rt(
    e,
    (o = A.audio) == null ? void 0 : o.language,
    (t = A.audio) == null ? void 0 : t.voiceNames,
    (n = A.audio) == null ? void 0 : n.rate,
    (s = A.audio) == null ? void 0 : s.volume
  );
}, GA = () => {
  const { settings: A } = N(), { audioToggledOn: e, setSyncedAudioToggledOn: o, syncedAudioToggledOnRef: t } = AA(), { callRcbEvent: n } = iA(), s = T(async (a) => {
    var c;
    a !== t.current && ((c = A.event) != null && c.rcbToggleAudio && (await n(
      M.TOGGLE_AUDIO,
      {
        currState: t.current,
        newState: !t.current
      }
    )).defaultPrevented || o((i) => !i));
  }, [t]), r = T(async (a) => {
    var i, u;
    if ((i = A.audio) != null && i.disabled || !t.current)
      return;
    let c = a;
    if ((u = A.event) != null && u.rcbStartSpeakAudio) {
      const f = await n(M.START_SPEAK_AUDIO, { textToRead: c });
      if (f.defaultPrevented)
        return;
      c = f.data.textToRead;
    }
    Ut(A, c);
  }, [A, t]);
  return {
    audioToggledOn: e,
    speakAudio: r,
    toggleAudio: s
  };
}, hA = () => {
  var C, p;
  const { settings: A } = N(), { messages: e, setSyncedMessages: o, syncedMessagesRef: t } = FA(), {
    setSyncedIsBotTyping: n,
    setUnreadCount: s,
    syncedIsScrollingRef: r,
    syncedIsChatWindowOpenRef: a
  } = AA(), { streamMessageMap: c, chatBodyRef: i, paramsInputRef: u } = $(), { scrollToBottom: f } = sA(), { callRcbEvent: d } = iA(), { speakAudio: w } = GA(), { playNotificationSound: g } = zA(), B = T((E, I = !1) => {
    var k, b;
    HA(E);
    let m = !0;
    const S = E[E.length - 1];
    if (!S)
      return;
    const O = S.sender.toUpperCase();
    O === "USER" && (m = !1), (k = A.general) != null && k.embedded && ye(i.current) && (m = !1), (a.current && !r.current || I) && (m = !1), m && g(), !I && (O !== "USER" && ((b = A.chatWindow) != null && b.autoJumpToBottom) || O === "USER" || !r.current) && setTimeout(() => f(), 1);
  }, [A, i, a, r, g, f]), D = T(async (E, I = "BOT", m = null) => {
    var q, K, L, x, G;
    if (typeof E != "string")
      throw new Error("Content must be of type string to simulate stream.");
    I = I.toUpperCase();
    let S = wA(E, I);
    if ((q = A.event) != null && q.rcbStartSimulateStreamMessage) {
      const H = await d(
        M.START_SIMULATE_STREAM_MESSAGE,
        { message: S }
      );
      if (H.defaultPrevented)
        return null;
      m = H.data.simulateStreamChunker || m, S = H.data.message;
    }
    n(!1);
    let O = 30;
    I === "BOT" ? O = (K = A.botBubble) == null ? void 0 : K.streamSpeed : O = (L = A.userBubble) == null ? void 0 : L.streamSpeed;
    const k = { ...S, content: "" };
    o((H) => [...H, k]), B(t.current);
    let b = S.content;
    m && (b = m(b));
    let U = 0;
    const R = b.length;
    S.sender.toUpperCase() === "BOT" && (a.current || (x = A.general) != null && x.embedded) && typeof S.content == "string" && S.content.trim() !== "" && w(S.content);
    const F = new Promise((H) => {
      const j = setInterval(() => {
        if (U >= R) {
          clearInterval(j), H();
          return;
        }
        o((Y) => {
          const _ = [...Y];
          for (let tA = _.length - 1; tA >= 0; tA--)
            if (_[tA].id === k.id) {
              const nA = b[U];
              nA && (k.content += nA, _[tA] = k), U++;
              break;
            }
          return _;
        });
      }, O);
    });
    return (r.current || !a.current) && s((H) => H + 1), await F, HA(t.current), (G = A.event) != null && G.rcbStopSimulateStreamMessage && await d(M.STOP_SIMULATE_STREAM_MESSAGE, { message: S }), I === "USER" && (u.current = E), S;
  }, [
    A,
    d,
    B,
    t,
    u,
    n,
    s,
    a,
    w
  ]), y = T(async (E, I = "BOT") => {
    var S, O, k;
    I = I.toUpperCase();
    let m = wA(E, I);
    if ((S = A.event) != null && S.rcbPreInjectMessage) {
      const b = await d(M.PRE_INJECT_MESSAGE, { message: m });
      if (b.defaultPrevented)
        return null;
      m = b.data.message;
    }
    return m.sender.toUpperCase() === "BOT" && (a.current || (O = A.general) != null && O.embedded) && typeof m.content == "string" && m.content.trim() !== "" && w(m.content), (r.current || !a.current) && s((b) => b + 1), (k = A.event) != null && k.rcbPostInjectMessage && await d(M.POST_INJECT_MESSAGE, { message: m }), o((b) => [...b, m]), B(t.current), I === "USER" && typeof E == "string" && (u.current = E), m;
  }, [
    A,
    d,
    B,
    u,
    t,
    a,
    w,
    s
  ]), P = T(async (E) => {
    var m;
    const I = t.current.find((S) => S.id === E);
    return !I || (m = A.event) != null && m.rcbRemoveMessage && (await d(M.REMOVE_MESSAGE, { message: I })).defaultPrevented ? null : (o(
      (S) => S.filter((O) => O.id !== E)
    ), B(t.current), s((S) => Math.max(S - 1, 0)), I);
  }, [
    d,
    (C = A.event) == null ? void 0 : C.rcbRemoveMessage,
    B,
    t,
    s
  ]), Q = T(async (E, I = "BOT") => {
    var S, O;
    if (I = I.toUpperCase(), !c.current.has(I)) {
      const k = wA(E, I);
      return (S = A.event) != null && S.rcbStartStreamMessage && (await d(M.START_STREAM_MESSAGE, { message: k })).defaultPrevented ? null : (n(!1), o((b) => [...b, k]), B(t.current), c.current.set(I, k.id), (r.current || !a.current) && s((b) => b + 1), k);
    }
    const m = { ...wA(E, I), id: c.current.get(I) };
    return (O = A.event) != null && O.rcbChunkStreamMessage && (await d(M.CHUNK_STREAM_MESSAGE, { message: m })).defaultPrevented ? null : (o(
      (k) => k.map((b) => b.id === m.id ? m : b)
    ), B(t.current, !0), m);
  }, [
    d,
    A.event,
    B,
    t,
    n,
    s,
    c
  ]), h = T(async (E = "BOT") => {
    var S;
    if (E = E.toUpperCase(), !c.current.has(E))
      return !0;
    const I = c.current.get(E);
    let m;
    for (let O = 0; O < 3; O++) {
      const k = t.current.find((b) => b.id === I);
      k && (m = k), await new Promise((b) => setTimeout(b, 20));
    }
    return (S = A.event) != null && S.rcbStopStreamMessage && (await d(M.STOP_STREAM_MESSAGE, { message: m })).defaultPrevented ? !1 : (c.current.delete(E), HA(t.current), E === "USER" && typeof (m == null ? void 0 : m.content) == "string" && (u.current = m.content), !0);
  }, [d, (p = A.event) == null ? void 0 : p.rcbStopStreamMessage, c, u]), v = T((E) => {
    o(E), B(E);
  }, [B]);
  return {
    simulateStreamMessage: D,
    injectMessage: y,
    removeMessage: P,
    streamMessage: Q,
    endStreamMessage: h,
    replaceMessages: v,
    messages: e
  };
};
let OA, lA, pA, gA = !1, aA = null;
const YA = () => {
  if (!OA) {
    const A = window.SpeechRecognition || window.webkitSpeechRecognition;
    OA = A != null ? new A() : null;
  }
  return OA;
}, Ft = (A, e, o, t, n, s, r) => {
  var a;
  (a = A.voice) != null && a.sendAsAudio ? zt(o, s) : jt(
    A,
    e,
    o,
    t,
    n,
    r
  );
}, jt = (A, e, o, t, n, s) => {
  var i, u, f;
  const r = YA();
  if (!r)
    return;
  if (!gA)
    try {
      gA = !0, r.lang = (i = A.voice) == null ? void 0 : i.language, r.start();
    } catch {
    }
  const a = (u = A.voice) == null ? void 0 : u.timeoutPeriod, c = (f = A.voice) == null ? void 0 : f.autoSendPeriod;
  r.onresult = (d) => {
    var g, B;
    clearTimeout(lA), lA = null, clearTimeout(pA);
    const w = d.results[d.results.length - 1][0].transcript;
    if (s.current) {
      const D = (g = A.chatInput) == null ? void 0 : g.characterLimit, y = s.current.value + w;
      D != null && D >= 0 && y.length > D ? t(y.slice(0, D)) : t(y), n(s.current.value.length);
    }
    a && (lA = setTimeout(async () => await xA(e, s), a)), (B = A.voice) != null && B.autoSendDisabled || (pA = setTimeout(o, c));
  }, r.onend = () => {
    gA ? (r.start(), !lA && a && (lA = setTimeout(async () => await xA(e, s), a))) : (clearTimeout(lA), lA = null, clearTimeout(pA));
  }, a && (lA = setTimeout(async () => await xA(e, s), a));
}, zt = (A, e) => {
  navigator.mediaDevices.getUserMedia({ audio: !0 }).then((o) => {
    if (aA = new MediaRecorder(o), !gA)
      try {
        gA = !0, aA.start();
      } catch {
      }
    aA.ondataavailable = (t) => {
      e.current && e.current.push(t.data);
    }, aA.onstop = () => {
      A(), o.getTracks().forEach((t) => t.stop());
    };
  }).catch((o) => {
    console.error("Unable to use microphone:", o);
  });
}, XA = () => {
  const A = YA();
  A && (gA = !1, A && A.stop(), aA && aA.state !== "inactive" && (aA.stop(), aA = null), clearTimeout(lA), lA = null, clearTimeout(pA));
}, Gt = (A, e) => {
  var t, n, s;
  const o = YA();
  (t = e.voice) != null && t.disabled || !((n = e.chatInput) != null && n.blockSpam) || !o || (A && !gA ? (gA = !0, (s = e.voice) != null && s.sendAsAudio ? aA == null || aA.start() : o.start()) : A || XA());
}, xA = async (A, e) => {
  var o;
  (o = e.current) != null && o.disabled || await A(), XA();
}, TA = () => {
  const { settings: A } = N(), { voiceToggledOn: e, setSyncedVoiceToggledOn: o, syncedVoiceToggledOnRef: t } = AA(), { callRcbEvent: n } = iA(), s = T(async (a) => {
    var c;
    a !== t.current && ((c = A.event) != null && c.rcbToggleVoice && (await n(
      M.TOGGLE_VOICE,
      {
        currState: t.current,
        newState: !t.current
      }
    )).defaultPrevented || o((i) => !i));
  }, [t]), r = T((a) => {
    Gt(a, A);
  }, [A]);
  return {
    voiceToggledOn: e,
    toggleVoice: s,
    syncVoice: r
  };
}, DA = () => {
  const { settings: A } = N(), {
    inputLength: e,
    setInputLength: o,
    textAreaDisabled: t,
    setSyncedTextAreaDisabled: n,
    textAreaSensitiveMode: s,
    setSyncedTextAreaSensitiveMode: r,
    syncedTextAreaDisabledRef: a,
    syncedTextAreaSensitiveModeRef: c
  } = AA(), { inputRef: i, chatBodyRef: u, prevInputRef: f } = $(), { callRcbEvent: d } = iA(), w = T(async (h) => {
    var v, C, p;
    if (a.current && i.current) {
      i.current.value = "";
      return;
    }
    if (i.current && f.current !== null) {
      const E = (v = A.chatInput) == null ? void 0 : v.characterLimit, m = ((C = A.chatInput) == null ? void 0 : C.allowNewline) ? h : h.replace(/\n/g, " ");
      if (E != null && E >= 0 && m.length > E ? i.current.value = m.slice(0, E) : i.current.value = m, (p = A.event) != null && p.rcbTextAreaChangeValue && (await d(
        M.TEXT_AREA_CHANGE_VALUE,
        { currValue: i.current.value, prevValue: f.current }
      )).defaultPrevented) {
        i.current.value = f.current;
        return;
      }
      f.current = i.current.value;
    }
  }, [a, i, f, A, d]), g = T((h) => {
    var v;
    (v = i.current) != null && v.disabled || setTimeout(() => {
      var C, p, E;
      (C = A.general) != null && C.embedded ? ye(u == null ? void 0 : u.current) && ((p = i.current) == null || p.focus()) : h !== "start" && ((E = i.current) == null || E.focus());
    }, 100);
  }, []), B = T(() => {
    i.current && i.current.focus();
  }, []), D = T(() => {
    i.current && i.current.blur();
  }, []), y = T(() => i && i.current ? i.current.value : "", []), P = T((h) => {
    h !== a.current && n((v) => !v);
  }, [a]), Q = T((h) => {
    h !== c.current && r((v) => !v);
  }, [c]);
  return {
    textAreaDisabled: t,
    setSyncedTextAreaDisabled: n,
    textAreaSensitiveMode: s,
    setSyncedTextAreaSensitiveMode: r,
    inputLength: e,
    setInputLength: o,
    getTextAreaValue: y,
    setTextAreaValue: w,
    updateTextAreaFocus: g,
    focusTextArea: B,
    blurTextArea: D,
    toggleTextAreaDisabled: P,
    toggleTextAreaSensitiveMode: Q
  };
}, Ce = BA({
  toasts: [],
  setSyncedToasts: () => {
  },
  syncedToastsRef: { current: [] }
}), be = () => vA(Ce), Yt = ({ children: A }) => {
  const [e, o, t] = rA([]);
  return /* @__PURE__ */ l(Ce.Provider, { value: { toasts: e, setSyncedToasts: o, syncedToastsRef: t }, children: A });
}, EA = () => {
  const { settings: A } = N(), { toasts: e, setSyncedToasts: o, syncedToastsRef: t } = be(), { callRcbEvent: n } = iA(), s = T(async (c, i) => {
    var g, B, D, y;
    let u = null;
    if (t.current.length >= (((g = A.toast) == null ? void 0 : g.maxCount) ?? 3)) {
      if ((B = A.toast) != null && B.forbidOnMax)
        return null;
      u = SA();
      let P = { id: u, content: c, timeout: i };
      if ((D = A.event) != null && D.rcbShowToast) {
        const Q = await n(M.SHOW_TOAST, { toast: P });
        if (Q.defaultPrevented)
          return null;
        P = Q.data.toast;
      }
      return o((Q) => [...Q.slice(1), P]), u;
    }
    u = SA();
    let w = { id: u, content: c, timeout: i };
    if ((y = A.event) != null && y.rcbShowToast) {
      const P = await n(M.SHOW_TOAST, { toast: w });
      if (P.defaultPrevented)
        return null;
      w = P.data.toast;
    }
    return o((P) => [...P, w]), u;
  }, [A, n]), r = T(async (c) => {
    var u;
    const i = t.current.find((f) => f.id === c);
    return !i || (u = A.event) != null && u.rcbDismissToast && (await n(M.DISMISS_TOAST, { toast: i })).defaultPrevented ? null : (o((f) => f.filter((d) => d.id !== c)), c);
  }, [n]), a = T((c) => {
    o(c);
  }, []);
  return {
    showToast: s,
    dismissToast: r,
    toasts: e,
    replaceToasts: a
  };
}, QA = () => {
  var R, F, q, K, L;
  const { settings: A } = N(), { paths: e, setSyncedPaths: o, syncedPathsRef: t } = bA(), { flowRef: n, streamMessageMap: s, paramsInputRef: r, keepVoiceOnRef: a } = $(), {
    setSyncedIsBotTyping: c,
    setSyncedTextAreaDisabled: i,
    setSyncedTextAreaSensitiveMode: u,
    blockAllowsAttachment: f,
    setBlockAllowsAttachment: d,
    setTimeoutId: w
  } = AA(), { callRcbEvent: g } = iA(), {
    endStreamMessage: B,
    injectMessage: D,
    removeMessage: y,
    simulateStreamMessage: P,
    streamMessage: Q
  } = hA(), { showToast: h, dismissToast: v } = EA(), { toggleChatWindow: C } = sA(), { updateTextAreaFocus: p, setTextAreaValue: E } = DA(), { syncVoice: I } = TA(), m = T(async (x) => {
    var G;
    if ((G = A.event) != null && G.rcbPostProcessBlock) {
      const H = await g(M.POST_PROCESS_BLOCK, {
        block: x
      });
      if (H.defaultPrevented)
        return null;
      x = H.data.block;
    }
    return x;
  }, [(R = A.event) == null ? void 0 : R.rcbPostProcessBlock, g]), S = T(async (x, G) => {
    var Y, _, tA, nA;
    if (!x)
      return;
    let H = n.current[x];
    if (!H) {
      c(!1);
      return;
    }
    if ((Y = A.event) != null && Y.rcbPreProcessBlock) {
      const PA = await g(M.PRE_PROCESS_BLOCK, {
        block: H
      });
      if (PA.defaultPrevented)
        return;
      H = PA.data.block;
    }
    c(!0), (_ = A.chatInput) != null && _.blockSpam && i(!0), u(!1);
    const j = {
      prevPath: G,
      currPath: x,
      goToPath: b,
      setTextAreaValue: E,
      userInput: r.current,
      endStreamMessage: B,
      injectMessage: D,
      removeMessage: y,
      simulateStreamMessage: P,
      streamMessage: Q,
      toggleChatWindow: C,
      showToast: h,
      dismissToast: v
    };
    await Qt(
      H,
      j,
      ((tA = A.botBubble) == null ? void 0 : tA.simulateStream) ?? !1,
      i,
      u,
      w,
      m
    ), c(!1), "chatDisabled" in H || i((nA = A.chatInput) == null ? void 0 : nA.disabled), "isSensitive" in H || u(!1), d(typeof H.file == "function"), p(x), I(a.current && !H.chatDisabled), x !== G && s.current.clear();
  }, [
    n,
    p,
    I,
    (F = A.botBubble) == null ? void 0 : F.simulateStream,
    (q = A.chatInput) == null ? void 0 : q.disabled,
    r,
    B,
    D,
    y,
    P,
    Q,
    C,
    h,
    v,
    a,
    s
  ]), O = T(() => t.current.length > 0 ? t.current.at(-1) ?? null : null, []), k = T(() => t.current.length > 1 ? t.current.at(-2) ?? null : null, []), b = T(async (x) => {
    var j;
    const G = O(), H = k();
    return (j = A.event) != null && j.rcbChangePath && (await g(M.CHANGE_PATH, {
      currPath: G,
      prevPath: H,
      nextPath: x
    })).defaultPrevented ? !1 : (o((Y) => [...Y, x]), await S(x, G), !0);
  }, [(K = A.chatInput) == null ? void 0 : K.blockSpam, (L = A.event) == null ? void 0 : L.rcbChangePath, S, g]), U = T((x) => {
    o(x);
  }, []);
  return {
    getCurrPath: O,
    getPrevPath: k,
    goToPath: b,
    blockAllowsAttachment: f,
    setBlockAllowsAttachment: d,
    paths: e,
    replacePaths: U,
    firePostProcessBlockEvent: m
  };
}, yA = () => {
  var R, F, q, K;
  const { settings: A } = N(), {
    endStreamMessage: e,
    injectMessage: o,
    removeMessage: t,
    simulateStreamMessage: n,
    streamMessage: s
  } = hA(), { syncedPathsRef: r } = bA(), { getCurrPath: a, getPrevPath: c, goToPath: i, firePostProcessBlockEvent: u } = QA(), {
    timeoutId: f,
    setSyncedTextAreaSensitiveMode: d,
    setSyncedTextAreaDisabled: w,
    setSyncedIsBotTyping: g,
    setBlockAllowsAttachment: B,
    setInputLength: D,
    syncedVoiceToggledOnRef: y,
    syncedTextAreaSensitiveModeRef: P
  } = AA(), { flowRef: Q, inputRef: h, keepVoiceOnRef: v, paramsInputRef: C } = $(), { showToast: p, dismissToast: E } = EA(), { callRcbEvent: I } = iA(), { syncVoice: m } = TA(), { setTextAreaValue: S } = DA(), { toggleChatWindow: O } = sA(), k = T(async (L) => {
    var H, j, Y, _, tA, nA;
    const x = a();
    if (!(!x || !Q.current[x])) {
      if (P.current) {
        if ((H = A == null ? void 0 : A.sensitiveInput) != null && H.hideInUserBubble)
          return;
        if ((j = A == null ? void 0 : A.sensitiveInput) != null && j.maskInUserBubble) {
          (Y = A.userBubble) != null && Y.simulateStream ? await n(
            "*".repeat(((_ = A.sensitiveInput) == null ? void 0 : _.asterisksCount) ?? 10),
            "USER"
          ) : await o("*".repeat(((tA = A.sensitiveInput) == null ? void 0 : tA.asterisksCount) ?? 10), "USER");
          return;
        }
      }
      (nA = A.userBubble) != null && nA.simulateStream ? await n(L, "USER") : await o(L, "USER");
    }
  }, [Q, a, A, o, n, P]), b = T(async (L, x = !0) => {
    var Y, _;
    if (L = L.trim(), L === "")
      return;
    x && await k(L), f && clearTimeout(f), h.current && (S(""), D(0));
    const G = a();
    if (!G)
      return;
    let H = Q.current[G];
    const j = await u(H);
    j && ((Y = A.chatInput) != null && Y.blockSpam && w(!0), v.current = y.current, m(!1), setTimeout(() => {
      g(!0);
    }, 400), d(!1), setTimeout(async () => {
      var PA;
      const tA = {
        prevPath: c(),
        currPath: a(),
        goToPath: i,
        setTextAreaValue: S,
        userInput: C.current,
        injectMessage: o,
        simulateStreamMessage: n,
        streamMessage: s,
        removeMessage: t,
        endStreamMessage: e,
        toggleChatWindow: O,
        showToast: p,
        dismissToast: E
      }, nA = r.current.length;
      await ve(j, tA), r.current.length === nA && ("chatDisabled" in H ? w(!!H.chatDisabled) : w(!!((PA = A.chatInput) != null && PA.disabled)), Be(H, tA, d), B(typeof H.file == "function"), m(v.current), g(!1));
    }, (_ = A.chatInput) == null ? void 0 : _.botDelay));
  }, [
    f,
    (R = A.chatInput) == null ? void 0 : R.blockSpam,
    (F = A.chatInput) == null ? void 0 : F.botDelay,
    (q = A.chatInput) == null ? void 0 : q.disabled,
    v,
    y,
    m,
    k,
    c,
    a,
    i,
    o,
    n,
    s,
    t,
    e,
    O,
    p,
    E,
    Q
  ]);
  return { handleSubmitText: T(async (L, x = !0) => {
    var H, j;
    L = L ?? ((H = h.current) == null ? void 0 : H.value), !((j = A.event) != null && j.rcbUserSubmitText && (await I(M.USER_SUBMIT_TEXT, { inputText: L, sendInChat: x })).defaultPrevented || !a()) && b(L, x);
  }, [I, a, b, h, (K = A.event) == null ? void 0 : K.rcbUserSubmitText]) };
}, MA = () => {
  var o;
  const { settings: A } = N();
  return fA(() => {
    var r;
    if (typeof window > "u" || !window.navigator)
      return !1;
    if (!((r = A.device) != null && r.applyMobileOptimizations))
      return !0;
    const t = navigator.userAgent, n = !/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(t), s = window.innerWidth >= 768;
    return n && s;
  }, [(o = A.device) == null ? void 0 : o.applyMobileOptimizations]);
}, Xt = ({ buttons: A }) => {
  var k, b, U, R, F, q, K, L, x, G, H;
  const e = MA(), { settings: o } = N(), { styles: t } = X(), {
    textAreaDisabled: n,
    textAreaSensitiveMode: s,
    inputLength: r,
    hasFlowStarted: a,
    setHasFlowStarted: c,
    setInputLength: i
  } = AA(), { inputRef: u } = $(), [f, d] = z(!1), [w, g] = z(!1), { handleSubmitText: B } = yA(), { setTextAreaValue: D } = DA(), y = {
    boxSizing: e ? "content-box" : "border-box",
    ...t.chatInputAreaStyle
  }, P = {
    outline: n ? "" : "none",
    boxShadow: n ? "" : `0 0 5px ${(k = o.general) == null ? void 0 : k.primaryColor}`,
    boxSizing: e ? "content-box" : "border-box",
    ...t.chatInputAreaStyle,
    // by default inherit the base style for input area
    ...t.chatInputAreaFocusedStyle
  }, Q = {
    cursor: `url("${(b = o.general) == null ? void 0 : b.actionDisabledIcon}"), auto`,
    caretColor: "transparent",
    boxSizing: e ? "content-box" : "border-box",
    ...t.chatInputAreaStyle,
    // by default inherit the base style for input area
    ...t.chatInputAreaDisabledStyle
  }, h = {
    color: "#989898",
    ...t.characterLimitStyle
  }, v = {
    color: "#ff0000",
    ...t.characterLimitReachedStyle
  }, C = n ? (U = o.chatInput) == null ? void 0 : U.disabledPlaceholderText : (R = o.chatInput) == null ? void 0 : R.enabledPlaceholderText, p = () => {
    n || d(!0);
  }, E = () => {
    d(!1);
  }, I = () => {
    g(!0);
  }, m = () => {
    g(!1);
  }, S = async (j) => {
    var Y;
    if (!w && j.key === "Enter") {
      if (j.shiftKey) {
        (Y = o.chatInput) != null && Y.allowNewline || j.preventDefault();
        return;
      }
      j.preventDefault(), await B();
    }
  }, O = (j) => {
    u.current && (D(j.target.value), i(u.current.value.length));
  };
  return /* @__PURE__ */ W(
    "div",
    {
      "aria-label": ((F = o.ariaLabel) == null ? void 0 : F.inputTextArea) ?? "input text area",
      role: "textbox",
      onMouseDown: (j) => {
        var Y;
        j.stopPropagation(), !a && ((Y = o.general) == null ? void 0 : Y.flowStartTrigger) === "ON_CHATBOT_INTERACT" && c(!0);
      },
      style: { ...t.chatInputContainerStyle },
      className: "rcb-chat-input",
      children: [
        s && ((q = o.sensitiveInput) != null && q.maskInTextArea) ? /* @__PURE__ */ l(
          "input",
          {
            ref: u,
            type: "password",
            className: "rcb-chat-input-textarea",
            style: n ? Q : f ? P : y,
            placeholder: C,
            onChange: O,
            onKeyDown: S,
            onFocus: p,
            onBlur: E,
            onCompositionStart: I,
            onCompositionEnd: m
          }
        ) : /* @__PURE__ */ l(
          "textarea",
          {
            ref: u,
            style: n ? Q : f ? P : y,
            rows: 1,
            className: "rcb-chat-input-textarea",
            placeholder: C,
            onChange: O,
            onKeyDown: S,
            onFocus: p,
            onBlur: E,
            onCompositionStart: I,
            onCompositionEnd: m
          }
        ),
        /* @__PURE__ */ W(oA, { children: [
          A == null ? void 0 : A.map(
            (j, Y) => /* @__PURE__ */ l(UA, { children: j }, Y)
          ),
          ((K = o.chatInput) == null ? void 0 : K.showCharacterCount) && ((L = o.chatInput) == null ? void 0 : L.characterLimit) != null && ((x = o.chatInput) == null ? void 0 : x.characterLimit) > 0 && /* @__PURE__ */ W(
            "div",
            {
              className: "rcb-chat-input-char-counter",
              style: r >= ((G = o.chatInput) == null ? void 0 : G.characterLimit) ? v : h,
              children: [
                r,
                "/",
                (H = o.chatInput) == null ? void 0 : H.characterLimit
              ]
            }
          )
        ] })
      ]
    }
  );
}, Wt = ({ buttons: A }) => {
  var t;
  const { settings: e } = N(), { styles: o } = X();
  return /* @__PURE__ */ W(
    "div",
    {
      "data-testid": "chatbot-footer-container",
      style: { ...o.footerStyle },
      className: "rcb-chat-footer-container",
      children: [
        /* @__PURE__ */ l("div", { className: "rcb-chat-footer", children: A == null ? void 0 : A.map(
          (n, s) => /* @__PURE__ */ l(UA, { children: n }, s)
        ) }),
        /* @__PURE__ */ l("span", { children: (t = e.footer) == null ? void 0 : t.text })
      ]
    }
  );
}, Kt = () => {
  var c, i, u, f, d, w, g;
  const { settings: A } = N(), { styles: e } = X(), { unreadCount: o } = AA(), { isChatWindowOpen: t, toggleChatWindow: n } = sA(), s = {
    backgroundImage: `linear-gradient(to right, ${(c = A.general) == null ? void 0 : c.secondaryColor},
			${(i = A.general) == null ? void 0 : i.primaryColor})`,
    ...e.chatButtonStyle
  }, r = {
    backgroundImage: `url(${(u = A.chatButton) == null ? void 0 : u.icon})`,
    fill: "#fff",
    width: 75,
    height: 75,
    ...e.chatIconStyle
  }, a = () => {
    var D;
    const B = (D = A.chatButton) == null ? void 0 : D.icon;
    return !B || typeof B == "string" ? /* @__PURE__ */ l(
      "span",
      {
        className: "rcb-toggle-icon",
        style: r
      }
    ) : B && /* @__PURE__ */ l("span", { className: "rcb-toggle-icon", children: /* @__PURE__ */ l(B, { style: r }) });
  };
  return /* @__PURE__ */ l(oA, { children: !((f = A.general) != null && f.embedded) && /* @__PURE__ */ W(
    "div",
    {
      "aria-label": ((d = A.ariaLabel) == null ? void 0 : d.chatButton) ?? "open chat",
      role: "button",
      style: s,
      className: `rcb-toggle-button ${t ? "rcb-button-hide" : "rcb-button-show"}`,
      onClick: () => n(!0),
      children: [
        a(),
        !((w = A.notification) != null && w.disabled) && ((g = A.notification) == null ? void 0 : g.showCount) && /* @__PURE__ */ l("span", { style: { ...e.notificationBadgeStyle }, className: "rcb-badge", children: o })
      ]
    }
  ) });
}, qt = () => {
  var w, g, B, D;
  const A = MA(), { settings: e } = N(), { styles: o } = X(), { isChatWindowOpen: t, toggleChatWindow: n } = sA(), [s, r] = z(!1), [a, c] = z(!1), [i, u] = z(0);
  Z(() => {
    var P, Q, h;
    const y = (P = e.tooltip) == null ? void 0 : P.mode;
    if (y === "ALWAYS")
      if (A) {
        let v;
        t ? v = (((Q = o.chatWindowStyle) == null ? void 0 : Q.width) ?? 375) - (((h = o.chatButtonStyle) == null ? void 0 : h.width) ?? 75) : v = 0, u(v), r(!0);
      } else
        r(!t);
    else y === "NEVER" ? r(!1) : y === "START" ? a ? r(!1) : (c(!0), r(!0)) : y === "CLOSE" && r(!t);
  }, [t]);
  const f = {
    transform: `translateX(-${i}px)`,
    right: (((w = o.chatButtonStyle) == null ? void 0 : w.width) ?? 75) + 40,
    bottom: 30,
    backgroundColor: (g = e.general) == null ? void 0 : g.secondaryColor,
    color: "#fff",
    ...o.tooltipStyle
  }, d = {
    borderColor: `transparent transparent transparent ${f.backgroundColor}`
  };
  return /* @__PURE__ */ l(oA, { children: !((B = e.general) != null && B.embedded) && /* @__PURE__ */ W(
    "div",
    {
      "data-testid": "chat-tooltip",
      style: f,
      className: `rcb-chat-tooltip ${s ? "rcb-tooltip-show" : "rcb-tooltip-hide"}`,
      onClick: () => n(!0),
      children: [
        /* @__PURE__ */ l("span", { children: (D = e.tooltip) == null ? void 0 : D.text }),
        /* @__PURE__ */ l("span", { className: "rcb-chat-tooltip-tail", style: d })
      ]
    }
  ) });
}, Jt = ({
  id: A,
  content: e,
  timeout: o
}) => {
  var d, w;
  const { settings: t } = N(), { styles: n } = X(), [s] = z({
    toastPromptStyle: { ...n.toastPromptStyle },
    toastPromptHoveredStyle: { ...n.toastPromptHoveredStyle }
  }), { dismissToast: r } = EA(), [a, c] = z(!1), i = {
    color: (d = t.general) == null ? void 0 : d.primaryColor,
    borderColor: (w = t.general) == null ? void 0 : w.primaryColor,
    ...s.toastPromptHoveredStyle
  };
  return Z(() => {
    if (o) {
      const g = setTimeout(() => {
        r(A);
      }, o);
      return () => clearTimeout(g);
    }
  }, [A, r, o]), typeof e == "string" ? /* @__PURE__ */ l(
    "div",
    {
      onMouseEnter: () => {
        c(!0);
      },
      onMouseLeave: () => {
        c(!1);
      },
      style: a ? i : s.toastPromptStyle,
      onMouseDown: async (g) => {
        var B;
        (B = t.toast) != null && B.dismissOnClick && (g.preventDefault(), await r(A));
      },
      className: "rcb-toast-prompt",
      children: e
    }
  ) : /* @__PURE__ */ l(oA, { children: e });
}, Vt = () => {
  var t, n, s, r;
  const { styles: A } = X(), { toasts: e } = be(), o = {
    bottom: (((t = A.footerStyle) == null ? void 0 : t.height) ?? 50) + (((n = A.chatInputContainerStyle) == null ? void 0 : n.height) ?? 70) + 20,
    width: 300,
    minWidth: (((s = A.chatWindowStyle) == null ? void 0 : s.width) ?? 375) / 2,
    maxWidth: (((r = A.chatWindowStyle) == null ? void 0 : r.width) ?? 375) - 50,
    ...A.toastPromptContainerStyle
  };
  return /* @__PURE__ */ l("div", { className: "rcb-toast-prompt-container", style: o, children: e.map((a) => /* @__PURE__ */ l(
    Jt,
    {
      id: a.id,
      content: a.content,
      timeout: a.timeout
    },
    a.id
  )) });
}, Qe = ({
  file: A,
  fileType: e,
  fileUrl: o
}) => {
  var r, a, c;
  const { settings: t } = N(), { styles: n } = X(), s = {
    backgroundColor: (r = t.general) == null ? void 0 : r.primaryColor,
    maxWidth: (a = t.userBubble) != null && a.showAvatar ? "65%" : "70%",
    ...n.mediaDisplayContainerStyle
  };
  return /* @__PURE__ */ l(oA, { children: o ? /* @__PURE__ */ W(oA, { children: [
    e === "image" && o && /* @__PURE__ */ l(
      "div",
      {
        style: s,
        className: "rcb-media-display-image-container rcb-media-entry",
        "data-testid": "media-display-image-container",
        children: /* @__PURE__ */ l("img", { src: o, alt: "Image Display", className: "rcb-media-display-image" })
      }
    ),
    e === "video" && o && /* @__PURE__ */ l(
      "div",
      {
        style: s,
        className: "rcb-media-display-video-container rcb-media-entry",
        "data-testid": "media-display-video-container",
        children: /* @__PURE__ */ W("video", { controls: !0, className: "rcb-media-display-video", children: [
          /* @__PURE__ */ l("source", { src: o, type: A.type }),
          "Your browser does not support the video tag."
        ] })
      }
    ),
    e === "audio" && o && /* @__PURE__ */ W(
      "audio",
      {
        "data-testid": "media-display-audio-container",
        style: {
          maxWidth: (c = t.userBubble) != null && c.showAvatar ? "65%" : "70%"
        },
        controls: !0,
        className: "rcb-media-display-audio rcb-media-entry",
        children: [
          /* @__PURE__ */ l("source", { src: o, type: A.type }),
          "Your browser does not support the audio tag."
        ]
      }
    )
  ] }) : /* @__PURE__ */ l(oA, {}) });
}, me = async (A) => {
  if (!A)
    return { fileType: null, fileUrl: null };
  const e = A.type.split("/")[0];
  if (!["image", "video", "audio"].includes(e))
    return { fileType: null, fileUrl: null };
  try {
    const o = await new Promise((t, n) => {
      const s = new FileReader();
      s.onload = () => t(s.result), s.onerror = () => n(new Error("File reading failed")), s.readAsDataURL(A);
    });
    return { fileType: e, fileUrl: o };
  } catch (o) {
    return console.error(o), { fileType: null, fileUrl: null };
  }
}, Zt = () => {
  var I, m, S, O, k, b;
  const { settings: A } = N(), { styles: e } = X(), {
    injectMessage: o,
    simulateStreamMessage: t,
    streamMessage: n,
    removeMessage: s,
    endStreamMessage: r
  } = hA(), { getCurrPath: a, getPrevPath: c, goToPath: i, blockAllowsAttachment: u } = QA(), { flowRef: f, inputRef: d } = $(), w = f.current, { showToast: g, dismissToast: B } = EA(), { callRcbEvent: D } = iA(), { toggleChatWindow: y } = sA(), { setTextAreaValue: P } = DA(), { handleSubmitText: Q } = yA(), h = {
    cursor: `url("${(I = A.general) == null ? void 0 : I.actionDisabledIcon}"), auto`,
    ...e.fileAttachmentButtonStyle,
    // by default inherit the base style
    ...e.fileAttachmentButtonDisabledStyle
  }, v = {
    backgroundImage: `url(${(m = A.fileAttachment) == null ? void 0 : m.icon})`,
    fill: "#a6a6a6",
    ...e.fileAttachmentIconStyle
  }, C = {
    backgroundImage: `url(${(S = A.fileAttachment) == null ? void 0 : S.icon})`,
    fill: "#a6a6a6",
    ...e.fileAttachmentIconStyle,
    // by default inherit the base style
    ...e.fileAttachmentIconDisabledStyle
  }, p = async (U) => {
    var L, x, G, H;
    const R = U.target.files;
    if (!R || (L = A.event) != null && L.rcbUserUploadFile && (await D(M.USER_UPLOAD_FILE, { files: R })).defaultPrevented)
      return;
    const F = a();
    if (!F)
      return;
    const q = w[F];
    if (!q)
      return;
    const K = q.file;
    if (K != null) {
      const j = [];
      for (let Y = 0; Y < R.length; Y++) {
        if (j.push(R[Y].name), !((x = A.fileAttachment) != null && x.showMediaDisplay))
          continue;
        const _ = await me(R[Y]);
        !_.fileType || !_.fileUrl || await o(/* @__PURE__ */ l(
          Qe,
          {
            file: R[Y],
            fileType: _.fileType,
            fileUrl: _.fileUrl
          }
        ), "USER");
      }
      await Q("📄 " + j.join(", "), (G = A.fileAttachment) == null ? void 0 : G.sendFileName), await K({
        userInput: (H = d.current) == null ? void 0 : H.value,
        prevPath: c(),
        currPath: a(),
        goToPath: i,
        setTextAreaValue: P,
        injectMessage: o,
        simulateStreamMessage: t,
        streamMessage: n,
        removeMessage: s,
        endStreamMessage: r,
        toggleChatWindow: y,
        showToast: g,
        dismissToast: B,
        files: R
      });
    }
  }, E = () => {
    var R, F;
    const U = u ? (R = A.fileAttachment) == null ? void 0 : R.icon : (F = A.fileAttachment) == null ? void 0 : F.iconDisabled;
    return !U || typeof U == "string" ? /* @__PURE__ */ l(
      "span",
      {
        className: u ? "rcb-attach-icon-enabled" : "rcb-attach-icon-disabled",
        style: u ? v : C
      }
    ) : U && /* @__PURE__ */ l("span", { className: u ? "rcb-attach-icon-enabled" : "rcb-attach-icon-disabled", children: /* @__PURE__ */ l(
      U,
      {
        style: u ? v : C
      }
    ) });
  };
  return /* @__PURE__ */ W(
    "label",
    {
      "aria-label": ((O = A.ariaLabel) == null ? void 0 : O.fileAttachmentButton) ?? "upload file",
      role: "button",
      className: u ? "rcb-attach-button-enabled" : "rcb-attach-button-disabled",
      style: u ? { ...e.fileAttachmentButtonStyle } : h,
      children: [
        /* @__PURE__ */ l(
          "input",
          {
            className: "rcb-attach-input",
            role: "file",
            type: "file",
            onChange: p,
            multiple: (k = A.fileAttachment) == null ? void 0 : k.multiple,
            accept: (b = A.fileAttachment) == null ? void 0 : b.accept,
            disabled: !u
          }
        ),
        E()
      ]
    }
  );
}, _t = () => {
  var D, y, P, Q, h, v;
  const { settings: A } = N(), { styles: e } = X(), { syncedTextAreaDisabledRef: o } = AA(), { inputRef: t } = $(), { textAreaDisabled: n, setTextAreaValue: s } = DA(), r = eA(null), a = eA(null), [c, i] = z(!1), u = {
    cursor: `url("${(D = A.general) == null ? void 0 : D.actionDisabledIcon}"), auto`,
    ...e.emojiButtonStyle,
    // by default inherit the base style
    ...e.emojiButtonDisabledStyle
  }, f = {
    backgroundImage: `url(${(y = A.emoji) == null ? void 0 : y.icon})`,
    fill: "#a6a6a6",
    ...e.emojiIconStyle
  }, d = {
    backgroundImage: `url(${(P = A.emoji) == null ? void 0 : P.icon})`,
    fill: "#a6a6a6",
    ...e.emojiIconStyle,
    // by default inherit the base style
    ...e.emojiIconDisabledStyle
  };
  Z(() => {
    const C = (I) => {
      const m = I.composedPath();
      r.current && !m.includes(r.current) && a.current && !m.includes(a.current) && i(!1);
    }, p = () => {
      if (r.current && a.current) {
        const I = a.current.getBoundingClientRect(), m = r.current.offsetHeight, S = I.top - m - 8;
        r.current.style.left = `${I.left}px`, r.current.style.top = `${S}px`;
      }
    }, E = () => {
      p();
    };
    return document.addEventListener("mousedown", C), window.addEventListener("resize", E), () => {
      document.removeEventListener("mousedown", C), window.removeEventListener("resize", E);
    };
  }, []);
  const w = (C, p) => {
    C.preventDefault(), t.current && (s(t.current.value + p), setTimeout(() => {
      const E = t.current;
      if (E) {
        E.focus();
        const I = E.value.length;
        E.setSelectionRange(I, I);
      }
    }, 50)), i(!1);
  }, g = (C) => {
    C.preventDefault(), o.current ? i(!1) : i((p) => !p);
  }, B = () => {
    var p, E;
    const C = n ? (p = A.emoji) == null ? void 0 : p.iconDisabled : (E = A.emoji) == null ? void 0 : E.icon;
    return !C || typeof C == "string" ? /* @__PURE__ */ l(
      "span",
      {
        className: `${n ? "rcb-emoji-icon-disabled" : "rcb-emoji-icon-enabled"}`,
        style: n ? d : f
      }
    ) : C && /* @__PURE__ */ l("span", { className: `${n ? "rcb-emoji-icon-disabled" : "rcb-emoji-icon-enabled"}`, children: /* @__PURE__ */ l(C, { style: n ? d : f }) });
  };
  return /* @__PURE__ */ W(oA, { children: [
    /* @__PURE__ */ l(
      "div",
      {
        "aria-label": ((Q = A.ariaLabel) == null ? void 0 : Q.emojiButton) ?? "emoji picker",
        role: "button",
        ref: a,
        className: `${n ? "rcb-emoji-button-disabled" : "rcb-emoji-button-enabled"}`,
        style: n ? u : { ...e.emojiButtonStyle },
        onMouseDown: g,
        children: B()
      }
    ),
    c && /* @__PURE__ */ l("div", { className: "rcb-emoji-button-popup", ref: r, children: (v = (h = A.emoji) == null ? void 0 : h.list) == null ? void 0 : v.map((C, p) => /* @__PURE__ */ l(
      "span",
      {
        className: "rcb-emoji",
        onMouseDown: (E) => w(E, C),
        children: C
      },
      p
    )) })
  ] });
}, $t = () => {
  var a, c, i;
  const { settings: A } = N(), { styles: e } = X(), { audioToggledOn: o, toggleAudio: t } = GA(), n = {
    backgroundImage: `url(${(a = A.audio) == null ? void 0 : a.icon})`,
    fill: "#fcec3d",
    ...e.audioIconStyle
  }, s = {
    backgroundImage: `url(${(c = A.audio) == null ? void 0 : c.iconDisabled})`,
    fill: "#e8eaed",
    ...e.audioIconStyle,
    // by default inherit the base style
    ...e.audioIconDisabledStyle
  }, r = () => {
    var f, d;
    const u = o ? (f = A.audio) == null ? void 0 : f.icon : (d = A.audio) == null ? void 0 : d.iconDisabled;
    return !u || typeof u == "string" ? /* @__PURE__ */ l(
      "span",
      {
        className: "rcb-audio-icon",
        "data-testid": "rcb-audio-icon",
        style: o ? n : s
      }
    ) : u && /* @__PURE__ */ l("span", { className: "rcb-audio-icon", "data-testid": "rcb-audio-icon", children: /* @__PURE__ */ l(
      u,
      {
        style: o ? n : s,
        "data-testid": "rcb-audio-icon-svg"
      }
    ) });
  };
  return /* @__PURE__ */ l(
    "div",
    {
      "aria-label": ((i = A.ariaLabel) == null ? void 0 : i.audioButton) ?? "toggle audio",
      role: "button",
      onMouseDown: async (u) => {
        u.preventDefault(), await t();
      },
      style: o ? { ...e.audioButtonStyle } : { ...e.audioButtonStyle, ...e.audioButtonDisabledStyle },
      children: r()
    }
  );
}, Ao = () => {
  var s, r;
  const { settings: A } = N(), { styles: e } = X(), { toggleChatWindow: o } = sA(), t = {
    backgroundImage: `url(${(s = A.header) == null ? void 0 : s.closeChatIcon})`,
    fill: "#e8eaed",
    stroke: "#e8eaed",
    ...e.closeChatIconStyle
  }, n = () => {
    var c;
    const a = (c = A.header) == null ? void 0 : c.closeChatIcon;
    return !a || typeof a == "string" ? /* @__PURE__ */ l(
      "span",
      {
        className: "rcb-close-chat-icon",
        "data-testid": "rcb-close-chat-icon",
        style: t
      }
    ) : a && /* @__PURE__ */ l("span", { className: "rcb-close-chat-icon", "data-testid": "rcb-close-chat-icon", children: /* @__PURE__ */ l(a, { style: t }) });
  };
  return /* @__PURE__ */ l(
    "div",
    {
      "aria-label": ((r = A.ariaLabel) == null ? void 0 : r.closeChatButton) ?? "close chat",
      role: "button",
      onMouseDown: (a) => {
        a.stopPropagation(), o(!1);
      },
      style: { ...e.closeChatButtonStyle },
      children: n()
    }
  );
}, eo = () => {
  const { notificationsToggledOn: A, toggleNotifications: e, playNotificationSound: o } = zA();
  return {
    notificationsToggledOn: A,
    toggleNotifications: e,
    playNotificationSound: o
  };
}, to = () => {
  var a, c, i;
  const { settings: A } = N(), { styles: e } = X(), { notificationsToggledOn: o, toggleNotifications: t } = eo(), n = {
    backgroundImage: `url(${(a = A.notification) == null ? void 0 : a.icon})`,
    fill: "#fcec3d",
    ...e.notificationIconStyle
  }, s = {
    backgroundImage: `url(${(c = A.notification) == null ? void 0 : c.iconDisabled})`,
    fill: "#e8eaed",
    ...e.notificationIconStyle,
    // by default inherit the base style
    ...e.notificationIconDisabledStyle
  }, r = () => {
    var f, d;
    const u = o ? (f = A.notification) == null ? void 0 : f.icon : (d = A.notification) == null ? void 0 : d.iconDisabled;
    return !u || typeof u == "string" ? /* @__PURE__ */ l(
      "span",
      {
        className: "rcb-notification-icon",
        "data-testid": "rcb-notification-icon",
        style: o ? n : s
      }
    ) : u && /* @__PURE__ */ l("span", { className: "rcb-notification-icon", "data-testid": "rcb-notification-icon", children: /* @__PURE__ */ l(
      u,
      {
        style: o ? n : s,
        "data-testid": "rcb-notification-icon-svg"
      }
    ) });
  };
  return /* @__PURE__ */ l(
    "div",
    {
      "aria-label": ((i = A.ariaLabel) == null ? void 0 : i.notificationButton) ?? "toggle notifications",
      role: "button",
      onMouseDown: async (u) => {
        u.preventDefault(), await t();
      },
      style: o ? { ...e.notificationButtonStyle } : { ...e.notificationButtonStyle, ...e.notificationButtonDisabledStyle },
      children: r()
    }
  );
}, oo = () => {
  var P, Q, h;
  const { settings: A } = N(), { styles: e } = X(), { injectMessage: o } = hA(), { inputRef: t } = $(), { voiceToggledOn: n, toggleVoice: s } = TA(), { setInputLength: r, setTextAreaValue: a, textAreaDisabled: c } = DA(), { handleSubmitText: i } = yA(), u = eA([]), [f, d] = z(!1);
  Z(() => {
    var v;
    (v = A.voice) != null && v.sendAsAudio ? (D(), u.current = []) : i();
  }, [f]), Z(() => {
    n ? Ft(
      A,
      s,
      B,
      a,
      r,
      u,
      t
    ) : XA();
  }, [n]);
  const w = {
    backgroundImage: `url(${(P = A.voice) == null ? void 0 : P.icon})`,
    fill: "#9aa0a6",
    ...e.voiceIconStyle
  }, g = {
    backgroundImage: `url(${(Q = A.voice) == null ? void 0 : Q.iconDisabled})`,
    fill: "#9aa0a6",
    ...e.voiceIconStyle,
    // by default inherit the base style
    ...e.voiceIconDisabledStyle
  }, B = () => {
    d((v) => !v);
  }, D = async () => {
    const v = new Blob(u.current, { type: "audio/wav" }), C = new File([v], "voice-input.wav", { type: "audio/wav" }), p = await me(C);
    !p.fileType || !p.fileUrl || await o(/* @__PURE__ */ l(
      Qe,
      {
        file: C,
        fileType: p.fileType,
        fileUrl: p.fileUrl
      }
    ), "USER");
  }, y = () => {
    var C, p;
    const v = n ? (C = A.voice) == null ? void 0 : C.icon : (p = A.voice) == null ? void 0 : p.iconDisabled;
    return !v || typeof v == "string" ? /* @__PURE__ */ l(
      "span",
      {
        className: `rcb-voice-icon${n && !c ? "-on" : ""}`,
        "data-testid": "rcb-voice-icon",
        style: n && !c ? w : g
      }
    ) : v && /* @__PURE__ */ l("span", { className: `rcb-voice-icon ${n && !c ? "on" : ""}`, children: /* @__PURE__ */ l(v, { style: n && !c ? w : g }) });
  };
  return /* @__PURE__ */ l(
    "div",
    {
      "aria-label": ((h = A.ariaLabel) == null ? void 0 : h.voiceButton) ?? "toggle voice",
      role: "button",
      onMouseDown: async (v) => {
        v.preventDefault(), !c && await s();
      },
      style: n && !c ? { ...e.voiceButtonStyle } : { ...e.voiceButtonStyle, ...e.voiceButtonDisabledStyle },
      className: n && !c ? "rcb-voice-button-enabled" : "rcb-voice-button-disabled",
      children: y()
    }
  );
}, no = () => {
  var B, D, y, P, Q, h, v, C;
  const { settings: A } = N(), { styles: e } = X(), { textAreaDisabled: o } = AA(), [t, n] = z(!1), { handleSubmitText: s } = yA(), r = {
    backgroundColor: (B = A.general) == null ? void 0 : B.primaryColor,
    ...e.sendButtonStyle
  }, a = {
    cursor: `url("${(D = A.general) == null ? void 0 : D.actionDisabledIcon}"), auto`,
    backgroundColor: (y = A.general) == null ? void 0 : y.primaryColor,
    ...e.sendButtonStyle,
    // by default inherit the base style
    ...e.sendButtonDisabledStyle
  }, c = {
    backgroundColor: (P = A.general) == null ? void 0 : P.secondaryColor,
    ...e.sendButtonStyle,
    // by default inherit the base style
    ...e.sendButtonHoveredStyle
  }, i = {
    backgroundImage: `url(${(Q = A.chatInput) == null ? void 0 : Q.sendButtonIcon})`,
    fill: "#fff",
    ...e.sendIconStyle
  }, u = {
    backgroundImage: `url(${(h = A.chatInput) == null ? void 0 : h.sendButtonIcon})`,
    fill: "#fff",
    ...e.sendIconStyle,
    // by default inherit the base style
    ...e.sendIconDisabledStyle
  }, f = {
    backgroundImage: `url(${(v = A.chatInput) == null ? void 0 : v.sendButtonIcon})`,
    fill: "#fff",
    ...e.sendIconStyle,
    // by default inherit the base style
    ...e.sendIconHoveredStyle
  }, d = () => {
    n(!0);
  }, w = () => {
    n(!1);
  }, g = () => {
    var E;
    const p = (E = A.chatInput) == null ? void 0 : E.sendButtonIcon;
    return !p || typeof p == "string" ? /* @__PURE__ */ l(
      "span",
      {
        className: "rcb-send-icon",
        "data-testid": "rcb-send-icon",
        style: o ? u : t ? f : i
      }
    ) : p && /* @__PURE__ */ l("span", { className: "rcb-send-icon", "data-testid": "rcb-send-icon", children: /* @__PURE__ */ l(
      p,
      {
        style: o ? u : t ? f : i
      }
    ) });
  };
  return /* @__PURE__ */ l(
    "div",
    {
      "aria-label": ((C = A.ariaLabel) == null ? void 0 : C.sendButton) ?? "send message",
      role: "button",
      onMouseEnter: d,
      onMouseLeave: w,
      onMouseDown: async (p) => {
        p == null || p.preventDefault(), !o && await s();
      },
      style: o ? a : t ? c : r,
      className: "rcb-send-button",
      children: g()
    }
  );
}, so = (A) => Object.values(V).includes(A), ro = (A, e) => {
  var c, i, u, f, d, w, g, B, D;
  const o = {
    [V.AUDIO_BUTTON]: (c = A.audio) == null ? void 0 : c.disabled,
    [V.CLOSE_CHAT_BUTTON]: (i = A.general) == null ? void 0 : i.embedded,
    [V.EMOJI_PICKER_BUTTON]: (u = A.emoji) == null ? void 0 : u.disabled,
    [V.FILE_ATTACHMENT_BUTTON]: (f = A.fileAttachment) == null ? void 0 : f.disabled,
    [V.NOTIFICATION_BUTTON]: (d = A.notification) == null ? void 0 : d.disabled,
    [V.SEND_MESSAGE_BUTTON]: !1,
    [V.VOICE_MESSAGE_BUTTON]: (w = A.voice) == null ? void 0 : w.disabled
  }, t = {}, n = (y) => y ? y.map((P) => {
    if (typeof P == "string") {
      if (so(P) && !o[P]) {
        if (t[P])
          return t[P];
        const Q = e[P];
        if (typeof Q == "function") {
          const h = Q();
          return t[P] = h, h;
        }
        return null;
      }
      return null;
    }
    return P;
  }).filter((P) => P !== null) : [], s = n((g = A.header) == null ? void 0 : g.buttons), r = n((B = A.chatInput) == null ? void 0 : B.buttons), a = n((D = A.footer) == null ? void 0 : D.buttons);
  return { header: s, chatInput: r, footer: a };
}, ao = () => /* @__PURE__ */ l($t, {}), co = () => /* @__PURE__ */ l(to, {}), io = () => /* @__PURE__ */ l(Ao, {}), lo = () => /* @__PURE__ */ l(oo, {}), uo = () => /* @__PURE__ */ l(no, {}), fo = () => /* @__PURE__ */ l(Zt, {}), wo = () => /* @__PURE__ */ l(_t, {}), go = () => {
  const { settings: A } = N(), e = fA(() => ({
    [V.CLOSE_CHAT_BUTTON]: () => io(),
    [V.AUDIO_BUTTON]: () => ao(),
    [V.NOTIFICATION_BUTTON]: () => co(),
    [V.EMOJI_PICKER_BUTTON]: () => wo(),
    [V.FILE_ATTACHMENT_BUTTON]: () => fo(),
    [V.SEND_MESSAGE_BUTTON]: () => uo(),
    [V.VOICE_MESSAGE_BUTTON]: () => lo()
  }), []), { header: o, chatInput: t, footer: n } = fA(() => ro(A, e), [A, e]), s = fA(() => o, [o]), r = fA(() => t, [t]), a = fA(() => n, [n]);
  return { headerButtons: s, chatInputButtons: r, footerButtons: a };
}, WA = () => {
  const { settings: A } = N(), { styles: e } = X(), { setSyncedMessages: o, syncedMessagesRef: t } = FA(), {
    isLoadingChatHistory: n,
    setIsLoadingChatHistory: s,
    hasChatHistoryLoaded: r,
    setHasChatHistoryLoaded: a
  } = AA(), { chatBodyRef: c } = $(), { callRcbEvent: i } = iA(), u = T(async () => {
    var w, g;
    const f = jA();
    if (!f || (w = A.event) != null && w.rcbLoadChatHistory && (await i(M.LOAD_CHAT_HISTORY, {})).defaultPrevented)
      return;
    s(!0);
    const d = ((g = c.current) == null ? void 0 : g.scrollHeight) ?? 0;
    kt(
      A,
      e,
      f,
      o,
      t,
      c,
      d,
      s,
      a
    );
  }, [
    A,
    e,
    i,
    t,
    c,
    s,
    a
  ]);
  return { isLoadingChatHistory: n, setIsLoadingChatHistory: s, hasChatHistoryLoaded: r, showChatHistory: u };
}, Bo = () => {
  var c, i, u;
  const { settings: A } = N(), { styles: e } = X(), { showChatHistory: o } = WA(), [t, n] = z(!1), s = {
    color: (c = A.general) == null ? void 0 : c.primaryColor,
    borderColor: (i = A.general) == null ? void 0 : i.primaryColor,
    ...e.chatHistoryButtonStyle,
    // by default inherit the base style
    ...e.chatHistoryButtonHoveredStyle
  };
  return /* @__PURE__ */ l("div", { className: "rcb-view-history-container", children: /* @__PURE__ */ l(
    "div",
    {
      onMouseEnter: () => {
        n(!0);
      },
      onMouseLeave: () => {
        n(!1);
      },
      style: t ? s : { ...e.chatHistoryButtonStyle },
      onMouseDown: (f) => {
        f.preventDefault(), o();
      },
      className: "rcb-view-history-button",
      role: "button",
      tabIndex: 0,
      children: /* @__PURE__ */ l("p", { children: (u = A.chatHistory) == null ? void 0 : u.viewChatHistoryButtonText })
    }
  ) });
}, pe = () => {
  var r;
  const { settings: A } = N(), { hasInteractedPage: e, setHasInteractedPage: o, hasFlowStarted: t, setHasFlowStarted: n } = AA(), s = T(() => {
    var c;
    if (o(!0), !t && ((c = A.general) == null ? void 0 : c.flowStartTrigger) === "ON_PAGE_INTERACT" && n(!0), !window.SpeechSynthesisUtterance) {
      console.info("Speech Synthesis API is not supported in this environment.");
      return;
    }
    const a = new window.SpeechSynthesisUtterance();
    a.text = "", a.onend = () => {
      window.removeEventListener("click", s), window.removeEventListener("keydown", s), window.removeEventListener("touchstart", s);
    }, window.speechSynthesis.speak(a);
  }, [t, (r = A.general) == null ? void 0 : r.flowStartTrigger]);
  return {
    hasInteractedPage: e,
    handleFirstInteraction: s
  };
}, vo = () => {
  var I, m, S, O, k;
  const A = MA(), { settings: e } = N(), {
    replaceMessages: o
  } = hA(), { goToPath: t } = QA(), {
    isBotTyping: n,
    isChatWindowOpen: s,
    hasFlowStarted: r,
    setUnreadCount: a,
    setSyncedIsChatWindowOpen: c,
    setSyncedTextAreaDisabled: i,
    setSyncedAudioToggledOn: u,
    setSyncedVoiceToggledOn: f,
    setSyncedIsScrolling: d,
    syncedIsScrollingRef: w,
    syncedIsChatWindowOpenRef: g
  } = AA(), { chatBodyRef: B } = $(), { viewportHeight: D, setViewportHeight: y, setViewportWidth: P, scrollToBottom: Q } = sA(), { setUpNotifications: h } = zA(), { handleFirstInteraction: v } = pe(), { showChatHistory: C } = WA(), p = eA(0), E = eA(!1);
  Z(() => (window.addEventListener("click", v), window.addEventListener("keydown", v), window.addEventListener("touchstart", v), () => {
    window.removeEventListener("click", v), window.removeEventListener("keydown", v), window.removeEventListener("touchstart", v);
  }), []), Z(() => {
    const b = B.current;
    if (!b)
      return;
    const U = () => {
      E.current || (E.current = !0, requestAnimationFrame(() => {
        var L, x;
        if (!b)
          return;
        const { scrollTop: R, clientHeight: F, scrollHeight: q } = b, K = R + F < q - (((L = e.chatWindow) == null ? void 0 : L.messagePromptOffset) ?? 30);
        d(K), w.current = K, K || (R + F >= q - 1 && (b.scrollTop = q - F - 1), (g.current || (x = e.general) != null && x.embedded) && a(0)), E.current = !1;
      }));
    };
    return b.addEventListener("wheel", U, { passive: !0 }), b.addEventListener("touchmove", U, { passive: !0 }), () => {
      b.removeEventListener("wheel", U), b.removeEventListener("touchmove", U);
    };
  }, []), Z(() => {
    var b, U, R;
    i((b = e.chatInput) == null ? void 0 : b.disabled), c((U = e.chatWindow) == null ? void 0 : U.defaultOpen), u((R = e.audio) == null ? void 0 : R.defaultToggledOn), setTimeout(() => {
      var F;
      f((F = e.voice) == null ? void 0 : F.defaultToggledOn);
    }, 1);
  }, []), Z(() => {
    var b;
    (b = e.notification) != null && b.disabled || h();
  }, [(I = e.notification) == null ? void 0 : I.disabled]), Z(() => {
    !w.current && (B != null && B.current) && Q();
  }, [n]), Z(() => {
    var b, U;
    if ((b = e.chatHistory) != null && b.disabled)
      Tt();
    else if (RA(e), jA().length > 0) {
      const F = wA(/* @__PURE__ */ l(Bo, {}), "SYSTEM");
      o([F]), (U = e.chatHistory) != null && U.autoLoad && C();
    }
  }, [(m = e.chatHistory) == null ? void 0 : m.storageKey, (S = e.chatHistory) == null ? void 0 : S.maxEntries, (O = e.chatHistory) == null ? void 0 : O.disabled]), Z(() => {
    var b;
    A || (b = e.general) != null && b.embedded || !navigator.virtualKeyboard || (navigator.virtualKeyboard.overlaysContent = !0, navigator.virtualKeyboard.addEventListener("geometrychange", (U) => {
      if (!U.target)
        return;
      const { x: R, y: F, width: q, height: K } = U.target.boundingRect;
      R == 0 && F == 0 && q == 0 && K == 0 ? (setTimeout(() => {
        var L;
        y((L = window.visualViewport) == null ? void 0 : L.height);
      }, 101), setTimeout(() => {
        var L, x;
        D != ((L = window.visualViewport) == null ? void 0 : L.height) && y((x = window.visualViewport) == null ? void 0 : x.height);
      }, 1001)) : setTimeout(() => {
        var L;
        y(((L = window.visualViewport) == null ? void 0 : L.height) - K);
      }, 101);
    }));
  }, [A]), Z(() => {
    var q, K, L, x;
    if (A)
      return;
    s && (y((q = window.visualViewport) == null ? void 0 : q.height), P((K = window.visualViewport) == null ? void 0 : K.width));
    const b = () => window.scrollTo({ top: 0, left: 0, behavior: "auto" }), U = () => p.current = window.scrollY, R = () => {
      var G, H;
      y((G = window.visualViewport) == null ? void 0 : G.height), P((H = window.visualViewport) == null ? void 0 : H.width);
    }, F = () => {
      var G;
      window.removeEventListener("scroll", b), window.removeEventListener("scroll", U), (G = window.visualViewport) == null || G.removeEventListener("resize", R);
    };
    return s ? (F(), document.body.style.position = "fixed", window.addEventListener("scroll", b), (L = window.visualViewport) == null || L.addEventListener("resize", R)) : (document.body.style.position = "static", F(), window.scrollTo({ top: p.current, left: 0, behavior: "auto" }), window.addEventListener("scroll", U), (x = window.visualViewport) == null || x.removeEventListener("resize", R)), F;
  }, [s, A]), Z(() => {
    var b;
    (r || ((b = e.general) == null ? void 0 : b.flowStartTrigger) === "ON_LOAD") && t("start");
  }, [r, (k = e.general) == null ? void 0 : k.flowStartTrigger]);
}, KA = {
  DEFAULT_URL: "https://cdn.jsdelivr.net/gh/tjtanjin/react-chatbotify-themes/themes",
  DEFAULT_EXPIRATION: "2592000",
  CACHE_KEY_PREFIX: "RCB_THEME_CACHE_DATA"
}, Po = KA.DEFAULT_URL, ho = KA.DEFAULT_EXPIRATION, Se = KA.CACHE_KEY_PREFIX, Do = (A, e, o) => {
  const t = localStorage.getItem(`${Se}_${A}_${e}`);
  if (!t)
    return null;
  try {
    const n = JSON.parse(t), s = (/* @__PURE__ */ new Date()).getTime(), r = Math.floor(s / 1e3);
    return n.cacheDate + o >= r ? n : null;
  } catch (n) {
    return console.error(`Unable to fetch cache for ${A}`, n), null;
  }
}, Eo = (A, e, o, t, n) => {
  const s = (/* @__PURE__ */ new Date()).getTime(), r = Math.floor(s / 1e3), a = {
    settings: o,
    inlineStyles: t,
    cssStylesText: n,
    cacheDate: r
  };
  localStorage.setItem(`${Se}_${A}_${e}`, JSON.stringify(a));
}, yo = async (A, e) => {
  const o = `${e}/${A}/meta.json`;
  try {
    const t = await fetch(o);
    return t.ok ? (await t.json()).version : (console.error(`Failed to fetch meta.json from ${o}`), null);
  } catch (t) {
    return console.error(`Failed to fetch meta.json from ${o}`, t), null;
  }
}, se = async (A, e) => {
  const { id: o, version: t, baseUrl: n = Po, cacheDuration: s = ho } = e, r = t || await yo(o, n);
  if (!r)
    return console.error(`Unable to find version for theme: ${o}`), { settings: {}, inlineStyles: {}, cssStylesText: "" };
  const a = Do(o, r, s);
  if (a) {
    const P = re(A, a.cssStylesText);
    return { settings: a.settings, inlineStyles: a.inlineStyles, cssStylesText: P };
  }
  const c = `${n}/${o}/${r}/styles.css`, i = `${n}/${o}/${r}/settings.json`, u = `${n}/${o}/${r}/styles.json`;
  let f = "";
  const d = await fetch(c);
  d.ok ? f = await d.text() : console.info(`Could not fetch styles.css from ${c}`);
  const w = await fetch(i);
  let g = {};
  w.ok ? g = await w.json() : console.info(`Could not fetch settings.json from ${i}`);
  const B = await fetch(u);
  let D = {};
  B.ok ? D = await B.json() : console.info(`Could not fetch styles.json from ${u}`), Eo(o, r, g, D, f);
  const y = re(A, f);
  return { settings: g, inlineStyles: D, cssStylesText: y };
}, re = (A, e) => e.split(new RegExp("(?<=})")).map((t) => {
  const n = t.trim();
  return n.startsWith("@import") || n.startsWith("@keyframes") || n.startsWith("@media") ? n : n ? `#${A} ${n}` : "";
}).join(`
`), Co = () => kA(ce), bo = () => kA(le), Qo = async (A, e, o, t) => {
  var a, c;
  let n = Co(), s = bo(), r = "";
  if (t != null)
    if (Array.isArray(t))
      for (const i of t) {
        const u = await se(A, i);
        n = uA(u.settings, n), s = uA(u.inlineStyles, s), r += u.cssStylesText;
      }
    else {
      const i = await se(A, t);
      n = uA(i.settings, n), s = uA(i.inlineStyles, s), r += i.cssStylesText;
    }
  return e != null && (n = uA(e, n)), o != null && (s = uA(o, s)), ((a = n.chatInput) == null ? void 0 : a.botDelay) != null && ((c = n.chatInput) == null ? void 0 : c.botDelay) < 500 && (n.chatInput.botDelay = 500), { settings: n, inlineStyles: s, cssStylesText: r };
}, uA = (A, e) => {
  const o = [{ source: A, target: e }];
  for (; o.length; ) {
    const t = o.pop();
    if (t == null)
      continue;
    const { source: n, target: s } = t;
    for (const r of Object.keys(n)) {
      const a = r;
      if (ae(n[a]))
        s[a] = n[a];
      else if (typeof n[a] == "object" && n[a] !== null && !Array.isArray(n[a]))
        (typeof s[a] != "object" || s[a] === null) && (s[a] = {}), o.push({ source: n[a], target: s[a] });
      else
        try {
          s[a] = n[a];
        } catch {
        }
    }
  }
  return e;
}, kA = (A) => {
  if (A === null || typeof A != "object")
    return A;
  const e = Array.isArray(A) ? [] : {}, o = [{ source: A, target: e }], t = /* @__PURE__ */ new WeakMap();
  for (t.set(A, e); o.length; ) {
    const n = o.pop();
    if (n == null)
      continue;
    const { source: s, target: r } = n;
    for (const a in s)
      if (Object.prototype.hasOwnProperty.call(s, a)) {
        const c = s[a];
        if (c && typeof c == "object")
          if (t.has(c))
            r[a] = t.get(c);
          else {
            const i = Array.isArray(c) ? [] : {};
            t.set(c, i), r[a] = i, o.push({ source: c, target: i });
          }
        else
          r[a] = c;
      }
  }
  return e;
}, Ie = () => {
  const { settings: A, setSettings: e } = N(), o = T((n) => {
    !n || Object.keys(n).length === 0 || e(kA(uA(n, A)));
  }, [A]), t = T((n) => {
    e(n);
  }, []);
  return {
    settings: A,
    replaceSettings: t,
    updateSettings: o
  };
}, Te = () => {
  const { styles: A, setStyles: e } = X(), o = T((n) => {
    !n || Object.keys(n).length === 0 || e(kA(uA(n, A)));
  }, [A]), t = T((n) => {
    e(n);
  }, []);
  return {
    styles: A,
    replaceStyles: t,
    updateStyles: o
  };
}, mo = (A) => {
  const { updateSettings: e } = Ie(), { updateStyles: o } = Te(), t = A == null ? void 0 : A.map((n) => n());
  Z(() => {
    let n = {}, s = {};
    t == null || t.forEach((r) => {
      r != null && r.settings && Object.keys(r == null ? void 0 : r.settings).length !== 0 && (n = uA(r.settings, n)), r != null && r.styles && Object.keys(r == null ? void 0 : r.styles).length !== 0 && (s = uA(r.styles, s));
    }), e(n), o(s);
  }, []);
}, po = ({
  plugins: A
}) => {
  var D, y, P, Q;
  const e = MA(), { settings: o } = N(), { styles: t } = X(), { hasFlowStarted: n, setHasFlowStarted: s } = AA(), { inputRef: r } = $(), {
    viewportHeight: a,
    viewportWidth: c,
    isChatWindowOpen: i
  } = sA(), { headerButtons: u, chatInputButtons: f, footerButtons: d } = go();
  vo(), mo(A);
  const w = fA(() => {
    var v;
    const h = "rcb-chatbot-global ";
    return (v = o.general) != null && v.embedded ? h + "rcb-window-embedded" : i ? h + "rcb-window-open" : h + "rcb-window-close";
  }, [o, i]), g = () => {
    var h, v;
    return !e && !((h = o.general) != null && h.embedded) ? {
      ...t.chatWindowStyle,
      borderRadius: "0px",
      left: "0px",
      right: "auto",
      top: "0px",
      bottom: "auto",
      width: `${c}px`,
      height: `${a}px`,
      zIndex: 1e4
    } : (v = o.general) != null && v.embedded ? { ...t.chatWindowStyle } : {
      ...t.chatWindowStyle,
      zIndex: 1e4
    };
  };
  return /* @__PURE__ */ l(oA, { children: (() => {
    var h, v;
    return e && ((h = o.device) == null ? void 0 : h.desktopEnabled) || !e && ((v = o.device) == null ? void 0 : v.mobileEnabled);
  })() && /* @__PURE__ */ W(
    "div",
    {
      onMouseDown: (h) => {
        var v, C;
        !n && ((v = o.general) == null ? void 0 : v.flowStartTrigger) === "ON_CHATBOT_INTERACT" && s(!0), e ? (C = r.current) == null || C.blur() : h == null || h.preventDefault();
      },
      className: w,
      children: [
        /* @__PURE__ */ l(qt, {}),
        /* @__PURE__ */ l(Kt, {}),
        i && !e && !((D = o.general) != null && D.embedded) && /* @__PURE__ */ W(oA, { children: [
          /* @__PURE__ */ l("style", { children: `
									html {
										overflow: hidden !important;
										touch-action: none !important;
										scroll-behavior: auto !important;
									}
								` }),
          /* @__PURE__ */ l(
            "div",
            {
              style: {
                position: "fixed",
                top: 0,
                left: 0,
                width: "100%",
                height: "100%",
                backgroundColor: "#fff",
                zIndex: 9999
              }
            }
          )
        ] }),
        /* @__PURE__ */ W("div", { style: g(), className: "rcb-chat-window", children: [
          ((y = o.general) == null ? void 0 : y.showHeader) && /* @__PURE__ */ l(nt, { buttons: u }),
          /* @__PURE__ */ l(wt, {}),
          /* @__PURE__ */ l(Vt, {}),
          ((P = o.general) == null ? void 0 : P.showInputRow) && /* @__PURE__ */ l(Xt, { buttons: f }),
          ((Q = o.general) == null ? void 0 : Q.showFooter) && /* @__PURE__ */ l(Wt, { buttons: d })
        ] })
      ]
    }
  ) });
}, Me = BA(void 0), ke = () => vA(Me), So = ({
  children: A
}) => {
  var u;
  const e = eA(""), o = eA({}), [t, n] = z({}), [s, r] = z({}), [a, c] = z(!1);
  Z(() => {
    c(!0);
  }, []);
  const i = async (f, d, w, g, B, D) => {
    e.current = f, o.current = d;
    const y = await Qo(f, w, g, B);
    D.current && (D.current.textContent = y.cssStylesText), n(y.settings), r(y.inlineStyles);
  };
  return a ? /* @__PURE__ */ l("div", { style: { fontFamily: (u = t.general) == null ? void 0 : u.fontFamily }, children: /* @__PURE__ */ l(Me.Provider, { value: { loadConfig: i }, children: /* @__PURE__ */ l(tt, { settings: t, setSettings: n, children: /* @__PURE__ */ l(ot, { styles: s, setStyles: r, children: /* @__PURE__ */ l(Yt, { children: /* @__PURE__ */ l(st, { botIdRef: e, flowRef: o, children: /* @__PURE__ */ l(ct, { children: /* @__PURE__ */ l(rt, { settings: t, children: /* @__PURE__ */ l(lt, { children: A }) }) }) }) }) }) }) }) }) : null;
}, Io = ({
  id: A,
  flow: e,
  settings: o,
  styles: t,
  themes: n,
  plugins: s,
  setConfigLoaded: r,
  styleRootRef: a
}) => {
  const c = fA(() => A, []), i = ke(), { flowRef: u } = $();
  u && u.current !== e && (u.current = e);
  const f = async () => {
    var d, w;
    (d = o.event) != null && d.rcbPreLoadChatBot && (await NA(
      M.PRE_LOAD_CHATBOT,
      { botId: c, currPath: null, prevPath: null },
      {
        flow: e,
        settings: o,
        styles: t,
        themes: n,
        plugins: s
      }
    )).defaultPrevented || (i != null && i.loadConfig && (await i.loadConfig(c, e, o, t, n, a), r(!0)), (w = o.event) != null && w.rcbPostLoadChatBot && NA(
      M.POST_LOAD_CHATBOT,
      { botId: c, currPath: null, prevPath: null },
      {
        flow: e,
        settings: o,
        styles: t,
        themes: n,
        plugins: s
      }
    ));
  };
  return Z(() => {
    f();
  }, [n]), null;
}, LA = ["Quickstart", "API Docs", "Examples", "Github", "Discord"], To = {
  start: {
    message: "Hello, I am Tan Jin 👋! Welcome to React ChatBotify, I'm excited that you are using our chatbot 😊!",
    transition: { duration: 1e3 },
    chatDisabled: !0,
    path: "show_options"
  },
  show_options: {
    message: "It looks like you have not set up a conversation flow yet. No worries! Here are a few helpful things you can check out to get started:",
    options: LA,
    path: "process_options"
  },
  prompt_again: {
    message: "Do you need any other help?",
    options: LA,
    path: "process_options"
  },
  unknown_input: {
    message: "Sorry, I do not understand your message 😢! If you require further assistance, you may click on the Github option and open an issue there or visit our discord.",
    options: LA,
    path: "process_options"
  },
  process_options: {
    transition: { duration: 0 },
    path: (A) => {
      let e = "";
      switch (A.userInput) {
        case "Quickstart":
          e = "https://react-chatbotify.com/docs/introduction/quickstart/";
          break;
        case "API Docs":
          e = "https://react-chatbotify.com/docs/api/bot_options";
          break;
        case "Examples":
          e = "https://react-chatbotify.com/docs/examples/basic_form";
          break;
        case "Github":
          e = "https://github.com/tjtanjin/react-chatbotify/";
          break;
        case "Discord":
          e = "https://discord.gg/6R4DK4G5Zh";
          break;
        default:
          return "unknown_input";
      }
      return A.injectMessage("Sit tight! I'll send you right there!"), setTimeout(() => {
        window.open(e);
      }, 1e3), "repeat";
    }
  },
  repeat: {
    transition: { duration: 3e3 },
    path: "prompt_again"
  }
}, No = ({
  id: A,
  flow: e,
  settings: o,
  styles: t,
  themes: n,
  plugins: s
}) => {
  const r = fA(() => A || "rcb-" + SA(), []), a = !e || Object.keys(e).length === 0 ? To : e, c = o || {}, i = t || {}, u = s || [], [f, d] = z(!1), w = ke(), g = eA(null), [B, D] = z(n || []);
  Z(() => {
    n && D(n);
  }, [n]);
  const y = () => /* @__PURE__ */ W(oA, { children: [
    /* @__PURE__ */ l(
      Io,
      {
        styleRootRef: g,
        id: r,
        flow: a,
        settings: c,
        styles: i,
        themes: B,
        plugins: u,
        setConfigLoaded: d
      }
    ),
    f && /* @__PURE__ */ l(po, { plugins: s })
  ] });
  return w == null ? /* @__PURE__ */ W(So, { children: [
    /* @__PURE__ */ l("style", { ref: g }),
    /* @__PURE__ */ l("div", { id: r, children: y() })
  ] }) : /* @__PURE__ */ W(oA, { children: [
    /* @__PURE__ */ l("style", { ref: g }),
    /* @__PURE__ */ l("div", { id: r, children: y() })
  ] });
}, Ro = () => {
  const { audioToggledOn: A, speakAudio: e, toggleAudio: o } = GA();
  return {
    audioToggledOn: A,
    speakAudio: e,
    toggleAudio: o
  };
}, Mo = () => {
  const { botIdRef: A } = $();
  return {
    getBotId: T(() => A.current, [A])
  };
}, Uo = () => {
  const { getBotId: A } = Mo();
  return {
    getBotId: A
  };
}, Fo = () => {
  const { isChatWindowOpen: A, toggleChatWindow: e, toggleIsBotTyping: o, scrollToBottom: t } = sA();
  return {
    isChatWindowOpen: A,
    toggleChatWindow: e,
    toggleIsBotTyping: o,
    scrollToBottom: t
  };
}, jo = () => {
  const { hasInteractedPage: A } = pe();
  return {
    hasInteractedPage: A
  };
}, ko = () => {
  const { replaceMessages: A } = hA(), { replacePaths: e } = QA(), { replaceToasts: o } = EA(), { hasFlowStarted: t } = AA(), { flowRef: n } = $(), { settings: s } = N(), r = T(() => {
    RA(s), A([]), o([]), e(["start"]);
  }, [A, o, e, RA]), a = T(() => (n == null ? void 0 : n.current) ?? {}, [n]);
  return {
    hasFlowStarted: t,
    restartFlow: r,
    getFlow: a
  };
}, zo = () => {
  const { hasFlowStarted: A, restartFlow: e, getFlow: o } = ko();
  return {
    hasFlowStarted: A,
    restartFlow: e,
    getFlow: o
  };
}, Go = () => {
  const {
    endStreamMessage: A,
    injectMessage: e,
    removeMessage: o,
    simulateStreamMessage: t,
    streamMessage: n,
    messages: s,
    replaceMessages: r
  } = hA();
  return {
    endStreamMessage: A,
    injectMessage: e,
    removeMessage: o,
    simulateStreamMessage: t,
    streamMessage: n,
    messages: s,
    replaceMessages: r
  };
}, Yo = () => {
  const {
    getCurrPath: A,
    getPrevPath: e,
    goToPath: o,
    paths: t,
    replacePaths: n
  } = QA();
  return {
    getCurrPath: A,
    getPrevPath: e,
    goToPath: o,
    paths: t,
    replacePaths: n
  };
}, Xo = () => {
  const { settings: A, replaceSettings: e, updateSettings: o } = Ie();
  return {
    settings: A,
    replaceSettings: e,
    updateSettings: o
  };
}, Wo = () => {
  const { styles: A, replaceStyles: e, updateStyles: o } = Te();
  return {
    styles: A,
    replaceStyles: e,
    updateStyles: o
  };
}, Ko = () => {
  const {
    textAreaDisabled: A,
    toggleTextAreaDisabled: e,
    textAreaSensitiveMode: o,
    toggleTextAreaSensitiveMode: t,
    getTextAreaValue: n,
    setTextAreaValue: s,
    focusTextArea: r,
    blurTextArea: a
  } = DA();
  return {
    textAreaDisabled: A,
    toggleTextAreaDisabled: e,
    textAreaSensitiveMode: o,
    toggleTextAreaSensitiveMode: t,
    getTextAreaValue: n,
    setTextAreaValue: s,
    focusTextArea: r,
    blurTextArea: a
  };
}, qo = () => {
  const { showToast: A, dismissToast: e, toasts: o, replaceToasts: t } = EA();
  return {
    showToast: A,
    dismissToast: e,
    toasts: o,
    replaceToasts: t
  };
}, Jo = () => {
  const { voiceToggledOn: A, toggleVoice: e } = TA();
  return {
    voiceToggledOn: A,
    toggleVoice: e
  };
}, Vo = () => {
  const { showChatHistory: A, hasChatHistoryLoaded: e } = WA();
  return {
    showChatHistory: A,
    getHistoryMessages: jA,
    setHistoryMessages: It,
    hasChatHistoryLoaded: e
  };
};
export {
  V as Button,
  So as ChatBotProvider,
  No as default,
  Co as getDefaultSettings,
  bo as getDefaultStyles,
  Ro as useAudio,
  Uo as useBotId,
  Vo as useChatHistory,
  Fo as useChatWindow,
  jo as useFirstInteraction,
  zo as useFlow,
  Go as useMessages,
  eo as useNotifications,
  Yo as usePaths,
  Xo as useSettings,
  Wo as useStyles,
  Ko as useTextArea,
  qo as useToasts,
  Jo as useVoice
};
